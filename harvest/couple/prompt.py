...
TEMPLATE = (
    ...
    "tip now; thin pale-green outlined line, fading = the tip over the last {trace_s:g} s; THICK SOLID pale-green "
    "arrow with a FILLED head = the motion the policy has committed for the next 0.5 s; thin DASHED pale-green arrow "
    "with an OPEN head = your previous correction still being applied (colour alone does not separate these two -- "
    "go by line style and head shape); short red / green / blue lines at the tip, no arrowhead = robot frame x / y "
    "/ z. On wrist images the overlay is only a small box in the top-right corner, using a cyan solid-filled arrow "
    "for committed motion and a magenta dashed-open arrow for the correction, same style rule.\n"
