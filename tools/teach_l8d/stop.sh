#!/bin/bash
# Stop ONLY the L8-D processes (pitfalls P27 / P57 / P70 / P76: no pkill -f, no pattern that can match a shell
# command line). Ours = environment IR_INST=teach_l8d_<tag> (Isaac chain) or TEACH_L8D_JOB=<name> (training / eval /
...
for d in /proc/[0-9]*; do
  ...
  hit=0
  if [ -z "$want" ]; then
    echo "$env_lines" | grep -qE "^(IR_INST=teach_l8d_[a-z0-9_]+|TEACH_L8D_JOB=[a-z0-9_]+)$" && hit=1
  else
    echo "$env_lines" | grep -qxE "(IR_INST=teach_l8d_$want|TEACH_L8D_JOB=$want)" && hit=1
  fi
  [ $hit = 1 ] && ours[$p]=1
done
for d in /proc/[0-9]*; do
