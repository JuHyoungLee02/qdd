#!/bin/bash
# E-STRIP8 closed-loop videos (prereg_strip8.md §5.5): per set (dev, oodh082, oodh088) one make_videos.py call with
...
for set in dev oodh082 oodh088; do
  specs=()
  for spec in "$@"; do
    A=${spec%%=*}; R=${spec#*=}
    case $set in dev) dirs="standard dr";; oodh082) dirs="standard_tz0.82";; oodh088) dirs="standard_tz0.88";; esac
    n=0
    for d in $dirs; do
      [ -d $R/$d ] || continue
      mkdir -p $S/${set}_$A/x
