#!/bin/bash
# Stop ONLY the E-PT processes (pitfalls P27 / P57 / P70 / P76: no pkill -f, no pattern that can match a shell
# command line). Ours = environment IR_INST=teach_pt_<tag> (Isaac chain) or TEACH_PT_JOB=<name> (training / eval /
...
for d in /proc/[0-9]*; do
  ...
  hit=0
  if [ -z "$want" ]; then
    echo "$env_lines" | grep -qE "^(IR_INST=teach_pt_[a-z0-9_]+|TEACH_PT_JOB=[a-z0-9_]+)$" && hit=1
  else
    echo "$env_lines" | grep -qxE "(IR_INST=teach_pt_$want|TEACH_PT_JOB=$want)" && hit=1
  fi
  [ $hit = 1 ] && ours[$p]=1
done
for d in /proc/[0-9]*; do
