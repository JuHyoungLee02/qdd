#!/bin/bash
# Run a harvest.teach_pt or teach_l8 module (train / merge / evaluate / tools) with a /data-only environment and a job tag.
# usage: py.sh <venv: train|vllm> <gpu or -> <job name> <code dir> <module or script> [args...]
# log -> /data/harvest/logs/teach_pt/<job>.log ; stop: tools/teach_pt/stop.sh <job>
source /data/harvest/env.sh
V=$1; G=$2; J=$3; C=$4; shift 4
L=/data/harvest/logs/teach_pt; mkdir -p $L
[ "$G" != "-" ] && export CUDA_VISIBLE_DEVICES=$G
export TEACH_PT_JOB=$J PYTHONPATH=$C
