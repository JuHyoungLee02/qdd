#!/bin/bash
# vLLM server for E-PT (same settings as the astra_solo dry run: bf16, max_model_len 12288, prefix caching,
# greedy decoding from the client). usage: vllm.sh <gpu> <model dir> <served name pt_*> <port> [gpu mem util]
# log -> /data/harvest/logs/teach_pt/vllm_<name>.log ; stop: tools/teach_pt/stop.sh <name>
source /data/harvest/env.sh
G=$1; M=$2; N=$3; PORT=$4; U=${5:-0.40}
L=/data/harvest/logs/teach_pt; mkdir -p $L
export CUDA_VISIBLE_DEVICES=$G TEACH_PT_JOB=$N
