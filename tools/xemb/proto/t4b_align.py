...
for sp in STARTS:
    ...
    r = minimize(total, th0, args=(train,), method="Nelder-Mead", options={"maxiter": 900, "xatol": 1e-5, "fatol": 1e-3})
    E, K = cam(r.x)
...
for f in train[:30]:
    ...
    syn.append(dict(f, g=s.astype(bool), dt_edge=cv2.distanceTransform((~edge).astype(np.uint8), cv2.DIST_L2, 3),
                    edge=np.c_[xs, ys]))
thp = best[1] + np.r_[np.radians([2, -2, 1.5]), [0.04, -0.03, 0.05], [0.08]]
r = minimize(total, thp, args=(syn,), method="Nelder-Mead", options={"maxiter": 900, "xatol": 1e-5, "fatol": 1e-4})
Es, Ks = cam(r.x)
