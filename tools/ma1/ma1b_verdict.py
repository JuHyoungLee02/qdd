"""prereg_ma1b §5 verdict (fixed before any A3d run): runs (none, a3d) x seeds 1, 2 on se2e_c1 with the motion line;
none_s1 / none_s2 = the motion-confirm checkpoints motion_s1 / motion_s2 (reuse checked bit for bit, prereg §3).

Inputs: per run the `stageb_train predict` jsonl of its last/ checkpoint on the FULL val set (1,799 snapshots; item
records, the same val keys in every run) and the transition flags (se2e_c1/transition_val.json).
  python tools/ma1/ma1b_verdict.py --pred none_s1=F a3d_s1=F none_s2=F a3d_s2=F --trans F --out JSON [--label L]
Effect of seed s = acc(a3d_s) - acc(none_s) (paired: same seed, same data order); POOLED effect = mean over the two
seeds. Snapshot-cluster bootstrap (10,000 draws, seed 0, the same resampled snapshots for all four runs, pooled
effect per draw), 95 % percentile. Rule: ADOPT iff pooled overall effect >= +0.02 AND its bootstrap 2.5 % bound > 0
AND pooled transition-stratum effect >= -0.01 (CMP_EPS 1e-12 on the thresholds; the bound strictly > 0). Reported,
outside the rule: NLL, steady stratum, per question, per source (RB1 / RB2), seed spread.
"""
...
SEEDS = ("s1", "s2")
SOURCES = ("RB1", "RB2")
MIN_GAIN, MAX_TRANS_DROP = 0.02, 0.01
...
def effects(m, key="acc"):
    per = {s: {st: m[f"a3d_{s}"][st][key] - m[f"none_{s}"][st][key] for st in STRATA} for s in SEEDS}
...
def bootstrap(items, flags):
    ...
    for name, mask in (("all", np.ones(len(keys), bool)), ("transition", trans), ("steady", ~trans)):
        ...
        for _ in range(N_BOOT):
            ...
            draws.append(0.5 * ((a["a3d_s1"] - a["none_s1"]) + (a["a3d_s2"] - a["none_s2"])))
...
def main():
    ...
    ap.add_argument("--pred", nargs=4, required=True, help="run=path for none_s1 a3d_s1 none_s2 a3d_s2")
    ...
    for c in RUNS:
        ...
        m[c]["per_source"] = {}
        for s in SOURCES:
            xs = [r for r in items[c] if r["key"].split("_")[0] == s]
            m[c]["per_source"][s] = metrics(xs, flags)["all"]
    ...
    src = {s: float(np.mean([m[f"a3d_{sd}"]["per_source"][s]["acc"] - m[f"none_{sd}"]["per_source"][s]["acc"]
    ...
    verdict = {"pooled_overall": pooled["all"], "lower_95": boot["all"][0], "pooled_transition": pooled["transition"],
               ...
               "adopt": bool(c_point and c_lower and c_trans)}
    out = {"label": a.label, "runs": m, "per_seed_effect": per, "pooled_effect": pooled, "seed_spread": spread,
           "per_seed_effect_nll": per_nll, "pooled_effect_nll": pooled_nll, "per_source_pooled": src,
           "bootstrap_95": boot, "verdict": verdict,
           "rule": {"min_gain": MIN_GAIN, "lower_bound_gt": 0.0, "max_transition_drop": MAX_TRANS_DROP,
                    "n_boot": N_BOOT, "seed": SEED, "cmp_eps": CMP_EPS},
           "n_val_snapshots": len(keysets[RUNS[0]]),
           "val_keys_summary_sha": {c: (summ[c] or {}).get("val_keys_sha") for c in RUNS}}
    json.dump(out, open(a.out, "w"), indent=1)
    print(json.dumps({"label": a.label, "verdict": verdict, "per_seed": per, "spread": spread,
                      "per_source": src}, indent=1))
