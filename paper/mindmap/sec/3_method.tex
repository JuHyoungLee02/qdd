...
\begin{itemize}
  ...
  \item \idea{(R7 13회차 D1--D3) 12회차 수정이 인용한 사전 등록 문장을 전체로 다시 대조하니: FLIP\_TH 초기값은 질문 평균 점수의 선형 보간 분위였음 $\rightarrow$ 사전 등록은 \texttt{question\_id@vN}별 split conformal(표본이 모자라면 문턱 없음). 카나리 기준일은 질문 판본을 안 봐 ser-A-min-2 판이 옛 기준과 비교됨 $\rightarrow$ 판본 바뀌면 새 기준일. 표류 의심 다음 날 깨끗한 카나리만으로 J5가 다시 켜짐 $\rightarrow$ 표류 의심 뒤엔 그날 이후 다시 맞춘 보정 파일이 있어야 켬. 수정 \tentative{진행 중}}
\end{itemize}
