...
E-SR1b, E-SR1c & 조건 강화로 먼 구간 준수 0.8 & \tentative{진행 중} \\
E-MAR & MolmoAct 성립 조건 관문 뒤 궤적 보조·궤적 조건 조종 & 관문 일부 통과, 조건 9개 준비 \tentative{예정} \\
...
\subsection{조이스틱 준수와 조건 강화 (E-SR0 끝, E-SR1b·E-SR1c 진행 중)}
\label{sec:plan_adh}
...
\toprule
조건 & \makecell{반사실\\전체} & \makecell{먼\\구간} & \makecell{접촉\\근처} & \makecell{그럴듯한\\수정} \\
\midrule
E-SR0 공개 실물 (움직임 줄, 두 시드) & \tentative{0.340} & --- & --- & --- \\
E-SR0 시뮬 (E-MA2 C0) & \tentative{0.431} & --- & --- & --- \\
E-SR1b 드롭아웃 + CFG & \todoexp & \todoexp & \todoexp & \todoexp \\
E-SR1b 되짚기 결정 라벨 & \todoexp & \todoexp & \todoexp & \todoexp \\
E-SR1c 먼 구간 반사실 분기 & \todoexp & \todoexp & \todoexp & \todoexp \\
E-SR1c 분기 + 권한 게이트 adaLN & \todoexp & \todoexp & \todoexp & \todoexp \\
\bottomrule
\end{tabular}
\caption{\textbf{조이스틱 준수 (E-SR0 결과, 조건 강화 예정).} 반사실 방향 준수율(우연 약 0.33). 기대: 조건 강화, 특히 먼 구간 반사실 분기 데이터가 먼 구간 준수율을 0.8 넘게 올리고 접촉 근처 능력은 유지한다. 성립하면 조이스틱 결정이 실행을 조종한다는 설계 전제가 서고 결합의 교정을 결정 토큰으로 보낼 수 있다. 미달이면 교정은 청크·손끝 기준에 머물고 구조 보장으로 간다.}
\label{tab:adh}
\end{table}
