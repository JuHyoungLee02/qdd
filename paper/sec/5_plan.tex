...
E-STRIP8 & 손으로 쓴 장면·요령 정보가 추론 때도 필요한가 & 사전 등록(무료), \tentative{진행 중} \\
결합 학습 V0--V6 & 의도 공유 규약 형식으로 상위·VLA를 함께 학습(§96 보충 2--4) & 계획, \tentative{예정} \\
E-VLA-E2E & VLA 끝-끝 학습 대 단계식(§96 보충 4) & \tentative{등록 전} \\
...
\bottomrule
\end{tabular}
\caption{\textbf{주 결과 (계획, 지연 충실 트랙; 동기 공정 트랙은 같은 형식으로 병기).} 기대: 학습 VLA와 결정층을 많이 학습한 조건은 standard 성공률이 높지만 RD가 크고, PaceNotes-B는 $\pi_{0.5}$ 미세조정보다 RD가 작다(H1). Astra를 끄면 RD가 커진다(H3). 학습량 축(C, A, A+RL, B)의 RD 순서는 학습이 분포 밖 하락을 키우는지(H2)를 보여 준다. 정지 시간은 첫 계획과 안전 대기 밖에서 0이어야 하며, 아니면 구현 결함으로 재실행한다.}
\label{tab:main}
\end{table*}
...
\subsection{결합 학습 계획: 의도 공유 규약 형식으로 (\tentative{계획})}
\label{sec:plan_coupled}
최종 상위 모델은 Qwen3.5-35B-A3B이고 8B는 경향만 먼저 보는 임시 대리다(정본 §96 보충 2). 상위는 반드시 의도 공유 규약의 형식(구간 의도와 검증 판정을 담은 결합 형식)으로 학습해야 한다. 규약의 메시지(상위 답, VLA 입력, VLA가 소유한 결정, 상위 검증 입력, 시간)를 기존 결합 코드와 정본에 대응시켜 보니 런타임 배선은 이미 끝나 있었다(정본 §96 보충 3). 모자란 것은 셋이다. 그리퍼 질문과 구간 줄을 넣은 판본의 VLA 재학습, 결합 형식의 상위 학습(시뮬 참값 라벨러와 결합 시스템이 간 상태의 DAgger, 8B 판 E-TEACH-L8C 뒤 35B), 명시적 검증 필드(사용자 결정 필요)다. 검증은 무료 사다리로 한다. V0 재학습 관문, V1 VLA 단독 DEV $\ge$ \tentative{4/12}, V2 결합 사전 실행 재실행, V3 L8C 오프라인, V4 결합 폐루프(학생 8B + VLA 대 8B 단독 동기 대 VLA 단독, DEV 40 짝), V5 35B 순이다. 유료 E-Couple(V6)은 사용자 허락 뒤에만 한다. 시뮬 선생 데이터는 본 학습 자리에만 쓰는 규칙(정본 §88·§89)은 그대로다. VLA 쪽은 끝-끝 학습(E-VLA-E2E)을 새로 등록해, 이산 행동 토큰 예측·예측 결정 조건 교과·의도 교란을 지금의 단계식과 비교한다. 비교 지표는 결정과 청크의 일치, 반사실 준수, 쥐기 시점(오프라인)과 VLA 단독·결합 고리의 폐루프 성공이다. 이와 별도로 \textbf{E-STRIP8}(무료)은 L8 프롬프트에 손으로 적은 장면·요령 정보(탁자 높이, 물체 크기, 잡기·놓기 요령, 탁자 높이에 그린 격자)가 추론 때도 필요한지를 잰다. 팔은 넷이다. 그대로(S-full), 학습·추론 모두 최소 요청(S-min), 학습 때만 특권 정보를 떨어뜨리기(S-drop), L8 어댑터에 최소 요청(S-full$\rightarrow$min)이다. 여기에 OOD-H에서 손으로 적은 높이가 틀린 팔(S-stale)을 더한다(\tentative{진행 중}).
