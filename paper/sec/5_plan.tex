...
이 절의 표는 모두 결과 예정 칸(\todoexp)이며, 판정 기준은 실행 전에 고정한다. 각 표의 캡션에 우리가 기대하는 결과와, 그 결과가 지지하거나 반박하는 주장을 한 문장씩 적는다. 기대와 다른 결과가 나와도 같은 표에 그대로 보고한다. 실험 순서는 E0(지연·결정성) $\rightarrow$ E0.5(오프라인 표 재생) $\rightarrow$ E1(보정) $\rightarrow$ E-M4 $\rightarrow$ E-M8c $\rightarrow$ 본 평가(RD)이며, 입력 절제는 E1과 병행한다. Astra의 필요성(E-Astra-necessity)은 오프라인 단계를 먼저 하고 유료 폐루프는 사용자 승인 뒤에 돈다. 공개 실물 데이터의 소규모 학습 시험(S-E2E, 규모 곡선, E-TC)은 수치 앞에 모두 파이프라인 시험임을 밝히며, 결정 입력 형식을 고르는 데에만 쓴다.
...
\paragraph{움직임 인터페이스 탐침(E-Astra-motion, \tentative{준비 중}).} \cref{sec:astra}의 두 경로 가운데 Astra가 어떤 방식으로 움직임에 개입할 때 잘하는지를 먼저 오프라인으로 본다. 같은 장면에서 Astra가 물체 기준 3D 경로를 그리게 하는 조건과, 이미지를 보며 말단을 한 걸음씩 조종하게 하는 조건을 비교하고, 3D는 깊이 센서 없이 RGB만으로 얻는 후보(지지 평면 광선 교차, 두 시점 삼각측량, 스테레오 점군과 가상 시점)를 함께 비교한다. 결과로 M4 느린 투표자와 물체 기준 경로 가운데 무엇을 먼저 구현할지 정한다.
