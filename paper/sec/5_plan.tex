...
\paragraph{정확도 진단(사전 등록).} 정확도 \tentative{0.72}의 원인을 가르려고 결과를 보기 전에 세 진단을 등록했다. (D1) 끝 체크포인트에서 상수 학습률로 \tentative{1,200스텝}을 더 돌려도 검증 결정 NLL이 문턱 아래로 내려가지 않아 ``이 학습률에서는 계산 한계가 아님''으로 판정했다. 앞선 개선은 대부분 학습률 감쇠 구간에서 나왔다. (D2) 학습 표본 1,000개로 과적합 시험을 하면 학습 부분집합 정확도가 \tentative{0.885}(검증 \tentative{0.56})로 사전 등록 구간 안에 들어 ``불확정''이었다. (D3) 오답 가운데 라벨 경계 근처 표본은 \tentative{34\%}로 문턱 50\%에 못 미쳐 ``라벨 모호성 한계가 아님''이었다. 0.1\,s 떨어진 이웃 프레임끼리 라벨이 달라지는 비율로 잡은 단일 프레임 정확도의 대략 상한은 \tentative{약 0.93}이다. 오답은 대부분 ``움직이는가''와 크기 한 칸 차이였고, 방향 부호가 반대인 경우는 드물었다(\tentative{8/155}). 사전 등록 규칙에 따른 종합 판정은 \emph{판정 불가}다. 같은 스텝에서 데이터 양을 바꾸는 규모 곡선을 이어서 등록해 돌리고 있으며(\tentative{진행 중}), 결정 입력이 한 시점뿐이어서 움직임의 시작과 멈춤이 애매하다는 가설(시간 맥락)은 \cref{sec:plan_temporal}의 2$\times$2 시험으로 가른다. 이 진단 수치도 파이프라인 시험의 것이라 결과로 쓰지 않는다.

\subsection{시간 맥락 시험 (E-TC, 계획)}
\label{sec:plan_temporal}
\cref{sec:temporal}의 과거 프레임 입력이 결정 정확도를 올리는지를 본다. 같은 공개 데이터(학습 37,484, 검증 층화 300), 같은 스텝 수와 학습률 스케줄에서 두 축을 교차한 2$\times$2로 학습한다. 첫 축은 이미지 입력(한 시점 대 카메라마다 $[t-0.3\,\text{s},\,t]$ 2프레임 비디오), 둘째 축은 움직임 텍스트 줄(없음 대 있음)이다. 움직임 줄은 기록된 관절·그리퍼 속도에서 뽑은 거친 구간(정지, 느림, 빠름)과 주 이동 방향 한 줄이며, 학습 때 일정 비율(\tentative{30\%})로 \texttt{unknown}으로 바꾸는 드롭아웃을 건다. 두 축을 한 격자에서 재는 이유는 둘이 같은 정보(속도)를 다른 경로로 주기 때문이다. 시드는 \tentative{2개 이상}이다. 두 변경 모두 결정 프롬프트 형식을 바꾸므로 조건마다 새 직렬화 판본으로 학습하고, 한 시점·줄 없음 조건도 같은 코드로 다시 학습해 기준으로 쓴다.

지표는 검증 결정 정확도 전체, \textbf{전환 층}(다음 0.33\,s 안에 결정 라벨이 바뀌는 스냅샷, 스냅샷 경계 표시와 같은 정의) 정확도, 나머지 정상 층 정확도, 질문별(\texttt{dir\_xy}, \texttt{dir\_z}, \texttt{mag}) 정확도, decide 지연 p95다. 전환 층을 따로 보는 이유는 GAP~\cite{gap2026}의 보고 때문이다. 시각과 고유감각을 단순히 이어 붙인 정책은 움직임 전환 구간에서 시각을 덜 쓰게 되어 시각만 쓴 정책보다 나빠질 수 있었다. 속도 입력은 ``지금 움직이면 다음에도 움직인다''는 지름길을 주므로, 정상 층 점수는 오르면서 우리가 고치려는 전환 순간은 나빠질 수 있다. 채택 규칙(\tentative{사전 등록 예정}): 기준 대비 전체 정확도가 \tentative{$+0.02$} 이상 오르고, 전환 층 정확도가 \tentative{0.01}보다 더 떨어지지 않으며, decide p95 증가가 \tentative{10\%} 이하인 조건만 채택한다. 여러 조건이 통과하면 전체 정확도가 가장 높은 것을 고른다. 아무 조건도 통과하지 못하면 입력은 한 시점으로 두고, 그 결과를 이 표에 그대로 보고한다. 이 시험도 공개 데이터의 휴리스틱 라벨을 쓰는 소규모 학습이라, 채택은 본 단계 B 입력 형식을 정하는 근거일 뿐 논문 결과는 본 데이터로 다시 잰다.
...
\midrule
한 시점(기준) & 없음 & \todoexp & \todoexp & \todoexp & \todoexp \\
한 시점 & 있음 & \todoexp & \todoexp & \todoexp & \todoexp \\
2프레임 비디오 & 없음 & \todoexp & \todoexp & \todoexp & \todoexp \\
2프레임 비디오 & 있음 & \todoexp & \todoexp & \todoexp & \todoexp \\
\bottomrule
\end{tabular}
\caption{\textbf{시간 맥락 2$\times$2 (계획).} 값은 검증 결정 정확도. 기대: 2프레임 비디오는 시각 토큰 수가 같아 p95를 거의 바꾸지 않으면서 전체와 전환 층 정확도를 함께 올린다. 움직임 줄은 정상 층을 쉽게 올리지만 전환 층에서는 이득이 작거나 손해일 수 있다. 비디오 행이 채택 규칙을 통과하면 ``한 시점 입력의 속도 부재가 결정 오답의 한 원인''을 지지하고, 통과하지 못하면 남은 차이를 데이터 규모 쪽으로 돌린다.}
\label{tab:temporal}
\end{table}
