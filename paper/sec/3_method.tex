...
\begin{itemize}
  ...
  \item \dirn{첫 실험 = Jev 보정 측정(E1) + ``마차 시험''(E2: 같은 텍스트 상태에서 룰 / Jev 촘촘 / Jev 목표 지정 / 겹침 켬·끔)}
\end{itemize}
...
\begin{itemize}
  ...
  \item \dirn{실험: 멈춤 / 요청 하나 / 겹침+최신 답 / (a)만 / (b)만 / (a)+(b). 핵심은 (a)+(b) 대 겹침+최신 답}
...
\begin{itemize}
  ...
  \item \idea{겹침 블렌딩 + jerk 최소화~\cite{lipo2025}. 단 겹침 평균은 정밀 작업 성공률을 깎음 $\rightarrow$ 같은 보기끼리만 섞고 마지막에 저크 필터}
\end{itemize}
...
\begin{itemize}
  ...
  \item \idea{상위 6편 모두 상위 모델이 결정하는 동안 로봇이 기다리거나 상위 모델이 루프 밖. 빠른 모델이 비정지 루프 가까이 도는 구조는 없음 $\rightarrow$ 우리 자리}
...
\begin{itemize}
  ...
  \item M5: \dirn{같은 보기끼리만 RTC식으로 섞고 마지막에 저크 제한}
