...
\midrule
\multicolumn{4}{@{}l}{\textbf{2단계: 한계가 트리거를 넘을 때만 --- 의도 공유 VLA 층}} \\
V0 & VLA 재학습(구간 의도 줄, 그리퍼 질문, 먼 구간 분기) & 재학습 관문(결정 정확도 비열등, 재적재 일치) & 보류 \\
V1 & VLA 단독 폐루프, DEV 12편(런타임 멈춤 고리 수정 뒤) & 성공 $\ge$ \tentative{4/12} & 보류 \\
V2 & 결합 형식 상위 학생과 결합 사전 실행 & 대조 판정·권한 궤적이 설계대로, 충돌 0 & 보류 \\
...
트리거가 걸리면 같은 장면에서 VLA 단독, 상위 단독(동기), 결합 계층을 짝지은 시드로 비교한다. 층은 standard와 DR, 표본은 DEV 레이아웃 시드 20개 $\times$ 층 2 = 조건당 40편이다. 주 지표는 성공률의 짝 차(레이아웃 시드 군집 부트스트랩 10,000회)이다. 판정 밖으로 로봇이 멈춘 시간, 편당 호출 수·비용, 답 나이, 대조 판정의 분포, 신선한 상위 답이 반대했는데도 진행된 되돌릴 수 없는 동작 수를 보고한다. 판정 초안은 결합이 트리거를 건 한계를 줄이면서(예: 벽시계, 기다림 실패) 상위 단독과 성공률이 비열등할 때 채택이다.
...
P1 & 먼 구간 분기가 실데이터에서도 준수 $\ge0.8$ & \tentative{0.536}(과소적합) \\
P3 & 결합이 트리거한 한계를 줄인다 & 트리거 뒤 V3 \\
P5 & 구간 계획이 맞고 VLA가 시점을 맞게 정함 & 구간 정답 \tentative{8/10}(계획대로 상태) \\
P7 & 도착 시 대조가 맞게 갈리고 도움이 됨 & 구현; 폐루프 미측정 \\
P9 & 접촉 근처 외부 명령 차단을 거리 권한이 책임짐 & $a=0$ 효과 0 시험 통과 \\
\bottomrule
\end{tabular}
\caption{\textbf{열린 전제.} 모두 검증 전이며 상태 칸의 값은 초기값이다. 2단계 전제의 나머지(권한 추정, 그리퍼 선택지, 정밀도 대가)는 층을 다시 넣을 때 함께 연다.}
\label{tab:premises}
\end{table}
