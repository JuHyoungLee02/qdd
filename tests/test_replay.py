...
def _h(la2_lo, c2pp_lo):
    """judgment-2 Holm result (e05.gain_holm shape) with the given Holm-level lower bounds."""
    return {k: {"reject": lo > 0, "lo": lo, "hi": lo + 0.05, "level": 0.975}
            for k, lo in (("la2", la2_lo), ("c2pp", c2pp_lo))}
...
def test_judgment1_narrows_claim():
    r = {"flip_rate_success": 0.01, "gain_la2": 0.005, "gain_c2pp": 0.004, "gain_holm": _h(-0.01, -0.01)}
    assert judge_e05(r)["claim"] == "narrow_to_b"


def test_judgment2_keeps_a():
    r = {"flip_rate_success": 0.08, "gain_la2": 0.03, "gain_c2pp": 0.01, "gain_holm": _h(0.005, -0.01)}
    assert judge_e05(r)["claim"] == "keep_a"


def test_judgment3_stabilizer():
    r = {"flip_rate_success": 0.08, "gain_la2": 0.01, "gain_c2pp": 0.01, "gain_holm": _h(-0.01, -0.01)}
    assert judge_e05(r)["claim"] == "a_as_stabilizer"


def test_uncovered_case_reported_not_forced():
    r = {"flip_rate_success": 0.01, "gain_la2": 0.03, "gain_c2pp": 0.0, "gain_holm": _h(0.01, -0.01)}
    assert judge_e05(r)["claim"] == "undecided"


def test_judgment4_c_flip_and_6_c5a3():
    r = {"flip_rate_success": 0.05, "gain_la2": 0, "gain_c2pp": 0, "gain_holm": _h(-1, -1),
...
def test_judgment9_layers():
    ...
    out = judge_e05({"flip_rate_success": 0.1, "gain_la2": 0, "gain_c2pp": 0, "gain_holm": _h(-1, -1),
