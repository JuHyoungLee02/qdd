...
def test_table_z_default_path_unchanged():
    ...
    # L8-D / L8-X added ws / lift / objset keywords; all default None and are only passed when set
    assert "table_z: float | None = None, ws=None, lift: float | None = None," in src
    assert "kw = {} if ws is None and lift is None and objset is None else" in src and "table_z=table_z, **kw)" in src
