...
| **누적(확정분)** | | **18,015.3원** = E-Astra-motion 6,933.4원(장부 `H/logs/astra_motion/cost.jsonl` 256행 합; 상한 15,000원) + 프롬프트 검진 2,093.9원(장부 `H/logs/prompt_health/astra_ledger.jsonl` 120행; 상한 3,000원) + E-Astra-solo 3,080.9원(장부 `H/logs/astra_solo/ledger.jsonl` 56행; 상한 5,000원) + E-ACC 5,907.1원(장부 `H/logs/eacc/astra_ledger.jsonl` charge 30행 + `astra_ledger_r2.jsonl` 91행; 상한 8,000원) + 이전 `[금액 미기록]` 분 | | |
