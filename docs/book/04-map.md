...
| `harvest/serialize.py`, `deccall_snap.py`, `labels_v2.py`, `qid.py`, `intent.py` | 직렬화(ser-A-min-3: `with_intent_last_step` 구간·움직임·`last_step` 줄, 구간 어휘 `SEGMENTS`·`SEGMENT_ACTIONS`)·결정 호출(그리퍼 질문 위치: `jevcall.GRIPPER`, `build_snapshot_request(gripper=True)`의 `phase` 뒤, `stageb_data.QUESTIONS` 끝 = expert 결정 칸 5, `se2e_data.ITEM_QUESTIONS`)·정답·질문 id·의도 라벨 규칙(`intent`: 그리퍼 §87·구간 §90, Task 12) |
...
| `tests/test_intent.py` | ser-A-min-3 의도 입력 시험(Task 12): 그리퍼 규칙·경계, 융합 DecCall 그리퍼 질문, 단계 B 항목·결정 칸, S-E2E 명령 라벨, 구간 줄 어휘·R2/S-E2E 규칙·상태 끝 순서 |
