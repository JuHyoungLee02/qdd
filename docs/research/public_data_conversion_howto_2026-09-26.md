...
3. **표본(T4 카메라, RB2 539편)**: 순운동학 EE 투영 점 **18,636**, 카메라 정보가 든 C′ **3,073**(카메라 없는 C′ 3,073은 그대로), 추적 점 6,746·궤적 2,638.
4. **안 한 것**: 렌더한 손끝을 추적 씨앗으로 쓰는 시험(T4 카메라가 곧바로 투영 라벨을 주므로 우선순위를 낮춤), RB1(편마다 머리 자세가 다름)·RB3로 넓히기(SAM 마스크에 GPU 필요).
...
|---|---|---|
| 관문 | 보류 실루엣 경계 거리 **2.32 px**(≤ 5 px 통과, 프레임 96–97.5 %) | 누수 **1/305 불통**(O-A DEV), OOD-H 0/285 |
| 정확도 | 투영 픽셀 약 2 px. 카메라 자세는 ± 약 4°·5 cm 모호 | 공개 보류분 C′ 목표 2.6–3.6 cm. 우리 DEV 접근 106.8 mm(영샷의 0.85배, NONE) |
| 수율(지금) | RB2 투영 점 18,636 + C′(카메라 정보) 3,073 | Franka 3,381 + RoboTwin 353 + RB2 3,073 + RBY1 254 |
| 수율(확장) | 관절 + URDF 메시가 있는 기체만(AI Worker 계열 RB1·RB3·전체 ROBOTIS 공개분 — 편당 자세 맞춤 필요) | 보정·EE가 있는 시뮬(Franka 수백만, RoboTwin 약 8만 편), 위에서 잡기 필터 뒤 |
