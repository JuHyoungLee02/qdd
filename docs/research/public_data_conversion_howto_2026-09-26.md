...
## 13. 작전 T — T1+T4 대 C′ (2026-09-27 03:00–12:53 KST, 최종 결정은 사용자)

사용자 원문(user-log 144):
- "일단 내가 시간을 더 줄테니까 계속 도전해보다가 10시간 줄게 안되면 잘되는 탑2에 다 몰아주고 그다음에 탑1에 다 몰아주는걸로 할게"
- "아니다 깊이 추정은 그냥 아닌게 맞는 것 같아 버리고 2개중에 고민하고 계속 각각을 최대로 발전시킨 후에 10시간 뒤에 최종 결정하자. 이걸 작전명 T라고 할게"
- 규칙(user-log 143 등): 버리기 전에 한 번은 다시 검증한다. 재검증으로 살아난 쪽이 있어도 기존 안을 바로 버리지 않고, 같은 조건으로 공정하게 한 번 더 비교한다.

작전 T의 목적은 최종 35B 상위 모델에 **쓸 수 있고 누수 없는 데이터**를 더 많이, 더 다양하게 주는 것이다(협업 보드 '원래 의도').

### 13.1 T1+T4 (좌표계 만들기) — 이번 창에서 한 것

1. **T1 + E-MAR-real filter v2(기존 RB2 라벨 9.6만 점)**:
   - 남긴 비율 73.5 %(E-MAR-real 74.5 %와 같음).
   - 보류 편 재투영 **42 px**(원시 포인팅 34 px보다 나쁨).
   - 필터는 다른 팔을 찍은 점만 거르고, 그리퍼 **어느 부위**를 찍느냐 하는 흔들림(씨앗 편향)은 줄이지 못했다.
2. **T4 실루엣 정렬(구현: `tools/xemb/urdf_fk.py` + `proto/t4*_align.py`)**:
   - 입력: AI Worker FFW-BG2 URDF와 ROBOTIS `ai_worker` 메시(팔 4–7번 링크 + 그리퍼), 원시 관절값으로 순운동학, SAM 3.1 'robot arm' 마스크 480장(RB2 160편 × 3프레임; 'robot gripper' 문구는 점수 0이라 쓸 수 없었음).
   - 목적 함수: 투영한 메시 실루엣 경계와 마스크 경계 사이 거리를 양방향으로 잰다. 뒤쪽 다른 로봇에 끌리지 않게 투영 주변 60 px 안만 본다.
   - 푸는 변수: 카메라 6자유도(+ 초점).
   - **결과(보류 79프레임, 편 단위 분할)**: 경계 거리 중앙 **2.32 px**, ≤ 5 px 프레임 **96.2 %**(초점을 ZED 사양 367로 고정하면 97.5 %), IoU 0.75, 초점 368 px. **관문 ≤ 5 px 통과.** 겹쳐 그림 `t4/t4b_fit_*.jpg`에서 빨강(투영)과 노랑(마스크) 윤곽이 겹친다.
   - **발견**: 명목 URDF 카메라만으로도 경계 거리 4.65 px였다. T1에서 본 21–42 px는 **카메라 오차가 아니라 포인팅 라벨의 잡음**이었다. RB2 머리 카메라는 처음부터 거의 맞았다.
   - **한계(합성 시험)**: 알려진 카메라로 만든 마스크에서, 어긋나게 시작해 되찾게 하면 경계 1.4–1.9 px까지 맞으면서도 자세는 4°·5–7 cm 차이가 남는다. 팔이 카메라에서 거의 같은 거리에 있어 회전과 평행 이동이 서로를 상쇄하기 때문이다. → **영상 투영(픽셀 라벨)은 정확하지만, 카메라 '자세' 숫자는 약 ± 4°·5 cm 불확실**하다. 카메라 정보가 든 C′의 자세 글에는 이 불확실도를 함께 적는다.
3. **표본(T4 카메라, RB2 539편)**: 초점을 사양 367로 고정한 T4c 카메라로 다시 만들었다. 순운동학 EE 투영 점 **18,660**, 카메라 정보가 든 C′ **3,073**(카메라 없는 C′ 3,073은 그대로), 추적 점 6,746·궤적 2,638. C′ 답의 물체 이름은 과제 문장("Put the X into the Y.")에서 뽑는다(`src_rb2.names_of`) — O-A 누수 1건의 'the object' 문구 원인을 없앴다.
4. **다른 AI Worker 리그로 넓히기(`proto/t4_rig.py`, 결과 `t4r/`)**: RB1(CoffeeClassification)·RB3(pickup_obj) 각 40편 × 4프레임, SAM 3.1 마스크(GPU 약 10분). 두 데이터셋 모두 상태에 머리 관절이 없다.
   - 편마다 따로 맞추기(학습 2–3장): 보류 6.1–6.4 px — 장수가 적어 과소구속.
   - **데이터셋당 카메라 1대**(편 20 %를 보류): **RB3 보류 2.87 px, ≤ 5 px 73 %**(15장, 명목 21.6 px에서 출발) / **RB1 5.48 px, 44 %**(9장). RB1은 SAM 팔 마스크가 프레임 절반에서 실패(마스크 중앙 0 px)해 쓸 수 있는 장이 적다.
   - 뜻: 머리 자세가 데이터셋 안에서 고정이면 T4는 다른 리그에도 그대로 옮겨 간다(RB3). 마스크 품질이 병목이다(RB1). 보류 장수가 적어(9–15장) 수치의 폭은 넓다.
5. **안 한 것**: 렌더한 손끝을 추적 씨앗으로 쓰는 시험(T4 카메라가 곧바로 투영 라벨을 주므로 우선순위를 낮춤), RB1·RB3 표본 생성(보류 장수를 늘린 확인 뒤).

### 13.2 C′ (카메라 없는 조종) — 이번 창에서 한 것

1. **풀**(위에서 잡기 30°/45°/60°):
   - Franka 93.7/97.6/100 % — 표본 50꾸러미에서 C′ 3,381. 전체 9.2만 꾸러미로 늘리면 수백만 행 [추정].
   - RoboTwin 100 % — 353(8과제 × 3편). 전체 약 8만 편 [추정].
   - RB2 3,073.
   - RBY1 40/47/62 %.
   - BEHAVIOR 31 %, 다만 기저가 멈춘 구간은 39 %뿐.
2. **E-OPEN8 O-A(오픈 데이터만, 옛 시뮬 예비, 등록 b057c5a)**:
   - DEV 접근 xy 중앙 **106.8 mm**(영샷 125.5의 0.85배) → 등록 판정 **NONE**.
   - 유효 JSON 1.0, 행동 정확도 낮음.
   - OOD-H 접근 3D 190.5 mm(L8 35.0 mm).
   - **좌표 누수 DEV 1/305**: x 0.074 m로 우리 상자 밖이고, 답에 공개 데이터의 'the object' 문구가 섞였다. → 누수 관문(0) **불통**. OOD-H는 0/285.
   - 공개 보류분 정확도: RB2 C′ 목표 2.6 cm, Franka C′ 3.6 cm, 점 9.8–11.2 px.
   - 뜻: 공개 C′만으로도 **형식·자기 정보 읽기는 배운다**(유효 JSON 1.0, 보류 C′ 2.6–3.6 cm). 하지만 우리 장면으로는 거의 옮겨 가지 않았다. O-B(섞기)는 지시대로 옛 시뮬에서 취소했고 L8-X에서 다시 한다.

### 13.3 T3 버리기 전 재검토 (사용자 규칙)

- **시험**: BEHAVIOR 178프레임(물체 532, 받침 면 115). MoGe-2 원시 오차는 물체 중심 17.4 cm, 평면 8.0 cm였다.
- **가장 싸고 유망한 고침**: 프레임마다 순운동학 그리퍼 점 하나로 척도를 맞추거나, 두 점으로 척도와 이동을 함께 맞췄다.
  - 결과: 19.0 / 11.6 cm, 27.9 / 14.0 cm로 **더 나빠졌다**.
  - 원인: 닻으로 쓴 순운동학 점과 그 픽셀의 보이는 겉면 깊이가 6.6 cm 어긋난다.
- **공정성 점검**:
  - 비교 대상: 단일 모델(MoGe-2)이다. Depth Anything 3는 1시간 예산 안에서 돌리지 못했다.
  - 관문(3 cm)은 GT 깊이 겉면 편향(6 cm)보다도 작아서, 단안 추정에게는 매우 높은 문턱이다.
- **판정**: 한 번 더 검증해도 불통이라 **T3 폐기를 유지**한다.
- **되살릴 조건**(다음 판을 열 수 있는 경우):
  - 여러 프레임 일관성으로 척도를 맞추는 판이 나온다.
  - DA3 metric 판이 3 cm에 가까워진다.
  - 이 둘 중 하나가 나오면, 기존 안과 같은 조건으로 공정 비교를 거친 뒤에만 되살린다.

### 13.4 점수판 (12:30 KST 제출용; 12:53 이후 사용자 결정)

| 항목 | **T1+T4** (좌표계 만들기) | **C′** (카메라 없는 조종) |
|---|---|---|
| 관문 | 보류 실루엣 경계 거리 **2.32 px**(≤ 5 px 통과, 프레임 96–97.5 %). 다른 리그: RB3 2.87 px(73 %), RB1 5.48 px(44 %, 마스크 병목) | 누수 **1/305 불통**(O-A DEV; 원인 'the object' 문구는 고침, 재측정은 L8-X), OOD-H 0/285 |
| 정확도 | 투영 픽셀 약 2 px. 카메라 자세는 ± 약 4°·5 cm 모호 | 공개 보류분 C′ 목표 2.6–3.6 cm. 우리 DEV 접근 106.8 mm(영샷의 0.85배, NONE) |
| 수율(지금) | RB2 투영 점 18,660 + C′(카메라 정보) 3,073 | Franka 3,381 + RoboTwin 353 + RB2 3,073 + RBY1 254 |
| 수율(확장) | 관절 + URDF 메시가 있는 기체만(AI Worker 계열 RB1·RB3·전체 ROBOTIS 공개분 — 머리가 고정이면 데이터셋당 카메라 1대로 충분, RB3에서 확인) | 보정·EE가 있는 시뮬(Franka 수백만, RoboTwin 약 8만 편), 위에서 잡기 필터 뒤 |
| 학습 효과 측정 | 아직 없음(표본 생성 직후) | O-A 예비: 형식은 배우나 우리 장면 전이는 약함. O-B는 L8-X에서 |
| 비용·복잡도 | SAM 마스크(GPU 약 10분/480장) + CPU 최적화 약 20분/리그. 메시와 URDF 필요 | 변환만(CPU). 가장 쌈 |
| 위험 | 자세 모호성. 메시 없는 기체에는 못 씀. 실데이터라 조명·사람 가림 | 좌표 누수(이미 1건), 다른 로봇 기저 숫자 암기, 위에서 잡기 편향 |
| 35B 데이터 기여 | **우리 로봇 계열의 실영상**에 정확한 픽셀 라벨·카메라 정보 C′ | 여러 기체·장면의 조종 형식. 양은 큼 |

**추천(Claude, 12:30에 확정)**
- 두 안은 **경쟁이 아니라 역할이 다르다.**
  - T1+T4는 '우리 로봇 실영상의 정확한 라벨'이다.
  - C′는 '다기체 조종 형식의 양'이다.
- 한 개를 골라야 한다면 **T1+T4**를 추천한다. 관문을 통과했고, 누수 위험이 없는 픽셀·투영 라벨이며, 우리 로봇 계열에 직접 쓰인다.
- C′는 버리지 않는다. 형식 분리(답 문구에서 공개 데이터 이름 제거, `camera: unknown` 요청 틀)를 고친 뒤, L8-X에서 같은 조건 짝 비교(E-OPEN8 O-B)로 다시 판정한다.
- **공정 비교 규칙**(user-log 143): 12:53 결정 때 한쪽이 뒤집히면 바로 버리지 않는다. 다음 조건으로 사전 등록한 짝 비교를 한 번 더 한다.
  - 같은 데이터 예산(T = 6,521행), 같은 L8-X 기본 데이터, 같은 816걸음·시드·DEV/OOD 세트.
  - 팔은 '기본 + T1T4 묶음' 대 '기본 + C′ 묶음'.
  - 판정은 E-PT 5.2 규칙.
