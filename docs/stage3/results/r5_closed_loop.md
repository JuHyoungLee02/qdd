...
- **(a) 결정 호출**(`models.py`): T_c = 0.33 s 계단식, 동시 in-flight ≤ N_max = ⌈d̂/T_c⌉+1([정정 R7 9회차, 정본 §74: 기준 조건 C2(VLM Stream)는 상한 없음·실측 in-flight 기록]), 조기 호출은 주기 슬롯 하나를 당겨 씀(호출 예산 고정, M4 §4.3). 한 호출의 답은 `t_start ≥ t_send + d̂`인 첫 스텝부터 H = 3개 스텝의 표가 된다(표마다 t_state가 달라 J3 합의 표 조건 충족).
