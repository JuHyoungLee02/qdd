...
- **(a) 결정 호출**(`models.py`): T_c = 0.33 s 계단식, 동시 in-flight ≤ N_max = ⌈d̂/T_c⌉+1([정정 R7 9회차, 정본 §74: 기준 조건 C2(VLM Stream)는 상한 없음·실측 in-flight 기록]), 조기 호출은 주기 슬롯 하나를 당겨 씀(호출 예산 고정, M4 §4.3). 한 호출의 답은 `t_start ≥ t_send + d̂`인 첫 스텝부터 H = 3개 스텝의 표가 된다(표마다 t_state가 달라 J3 합의 표 조건 충족). [정정 R7 10회차 D1, 정본 §75: 이것은 H = 3의 규칙이다. H = 1(E-M4 비교 칸)은 사전 등록대로 "같은 스텝을 앞당겨 묻기" — 한 호출의 답이 시작 시각이 [t_send + d̂, t_send + lead_max]인 모든 스텝의 표(`m4.early_ask_steps`, 기본 lead_max 1.0 s, `closed --m4-lead-max`). 이 lead_max는 아래 §59 호출 방식 "lead"(vLLM 캐시 채우기)와 다른 뜻]
...
- **(b) M4 겹침 확정**(`m4.py`, M4 §4.1–§4.2, §4.6 충실 최소판): 슬롯 = (스텝, 질문), option_key로 셈. premise epoch 낮은 표 폐기, STALE_MAX 1.5 s 폐기, 이미 시작한 스텝(FROZEN)·확정 스텝은 불변(log_only), 도전 선택은 유예 창 W(비가역 보기 W+1)를 지나야 교체, (b) ≠ OK가 현 선택 뒤에 오면 즉시 교체(gate_hard), 앞쪽부터만 확정(LA-2 또는 3표 이상 γ ≥ 0.67 — [정정 R7 9회차, 정본 §74: 0.67 = 정확히 2/3("3표 중 2"), `m4.share_at_least` 정수 비교, E0.5 재생과 같은 함수; STALE_MAX 1.5 s·FROZEN log_only는 합의 원장 조건만, C2는 5 s 타임아웃·매 틱 가장 새 응답]), 순서형 mag는 τ = 1 칸 허용([정정 R7 8회차, 정본 §73: 접촉 근처(목표 ≤ 5 cm 또는 접촉)는 τ = 0 — `core.near_contact`를 `on_vote`·`try_commit_prefix`에 넘김]), flip_score(TV 거리) 기록(FLIP_TH는 E0.5 전 끔), 확정 못 한 채 시작한 스텝은 가확정 실행 후 "unconfirmed-executed"로 셈. (b) 결과: OK/LAG/DEVIATE/CONTRADICT, DEVIATE·CONTRADICT → epoch+1 + 미래 미확정 슬롯 재개방 + 조기 호출, CONTRADICT는 직전 확정 행동 유지(정지 아님). 확률 게이트·J5는 E1 전이라 끔(§6).
