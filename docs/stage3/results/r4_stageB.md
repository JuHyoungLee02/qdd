...
- **GPU 스모크·GPU 지연은 못 했다**: GPU 3은 다른 에이전트의 vLLM이 121 GB를 쓰고 있어 규칙상 CPU로 했다. 코디네이터가 중간에 "GPU 2 = 학습 전용"(user-log 62)으로 바꿨다고 알려 왔으나, 이 작업 지시문·사용자 메모리의 "GPU 2 금지"와 충돌하고 실행 권한 판정에서 거부되어 **돌리지 않았다** → 사용자 확인 필요(열린 문제 1). 기본 크기 expert의 10스텝 샘플링은 CPU 8스레드 p50 0.30 s(참고치, GPU 값 아님).
...
파드: 코드 사본 `/data/harvest/code_stageB`, 스모크 스크립트 `/data/harvest/ckpt/stageB/smoke_cpu.sh`, 산출 `/data/harvest/ckpt/stageB/smoke_tiny_cpu/`(log.jsonl, ckpt/, img/), 표준 출력 `smoke_tiny_cpu.out`.
...
## 7. 스모크 (파드 CPU, `smoke_tiny_cpu`, 2026-09-25 18:12–18:13 UTC 파드 시계 기준)
...
1. **GPU 스모크·GPU 지연 미실행**: 실제 4B 백본(BF16) + 기본 크기 expert로 50스텝, expert 10스텝 GPU 지연, 문맥 순전파 포함 전체 지연을 재야 R4 통과 조건("추론 속도")이 채워진다. GPU 3은 vLLM(121 GB) 사용 중이라 못 했고, GPU 2 사용은 이 작업 규칙·사용자 메모리("GPU 2 금지")와 코디네이터의 새 배분(user-log 62)이 충돌해 권한 판정에서 막혔다. 준비된 명령: `CUDA_VISIBLE_DEVICES=<GPU> python -m harvest.train.stageb_train smoke --backbone qwen --device cuda --out /data/harvest/ckpt/stageB/smoke_qwen4b --n 40 --steps 50 --batch 4 --lr 2e-4 --lr-heads 5e-4 --width 768 --depth 8 --heads 12`(코드 `/data/harvest/code_stageB`, `venv_train`, `TORCH_DISABLE_NATIVE_JIT=1`). **사용자가 GPU 2 사용을 확인해 주면 바로 돌린다.**
2. **처리량**: 한 표본 = 문맥 1회 + 질문 5회 순전파(각각 같은 이미지 2장 재인코딩). 본 학습 규모에서는 질문 5개를 한 서열로 묶거나 이미지 인코딩을 공유해야 한다(단계 A §7과 같은 문제, R3 묶음 처리와 함께).
3. **런타임 한 번 호출(§58, R5)**: 지금은 결정(질문별 트라이 호출)과 행동(문맥 순전파 + expert)이 다른 순전파다. vLLM 서빙과 expert를 한 서버에서 묶으려면 vLLM이 은닉 상태를 내주거나(현재 미지원 [미확인]) HF 순전파를 따로 둬야 한다 — 지연 예산(§49·§59 0.33 s)과 함께 R5에서 정할 일.
