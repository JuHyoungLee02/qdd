...
3. 파일럿 420편(11:16–12:17Z, GPU 1·0에 2+2프로세스) → 검사·분석(§3) → 이상 없음 → 본 생성은 파일럿 작업자가 끝나는 대로 자리를 이어받아 자동 시작(같은 큐라 파일럿 편은 그대로 본 데이터의 일부). [→ 정정 2026-09-25 23:58 UTC, R7 35회차 D-2: 실제 순서는 다르다 — 작업자 w5가 12:00:24Z에 GPU 1에서(결정성 확인 자리를 이어받아) 본 생성을 시작했고 첫 본 편(standard/mug_tray/P0 시드 10030)은 12:03:32Z에 기록됐다. 파일럿 검사·분석은 12:20:26Z로 본 생성이 이미 17분 돈 뒤였으므로 **관문이 아니었다**(`workers.log`).]
4. 본 생성 12:00–21:56Z(w5 12:00, w1·w3 12:15, w2 12:16, w4 12:32 시작 — `workers.log`) [→ 정정 2026-09-25 23:58 UTC, R7 35회차 D-2: 앞 판은 12:15]: GPU 1에 3프로세스(w1·w2·w5) + GPU 0에 2프로세스(w3·w4) = 최대 5. 각 자리는 6개 작업((변형, 섭동 띠))을 돌려 순서로(standard 먼저/dr 먼저) 돈다. 결정성 확인 6프로세스는 이 자리 안에서 차례로 돌았다(동시 5 넘지 않음).
...
- 유효 편 한 편 평균 293프레임(9.7 s 시뮬 [→ 정정 2026-09-25 23:58 UTC, R7 35회차 N174: 유효 편 시뮬 시각 평균 9.725 s]), 결정 항목 약 30.
...
- **실패의 91 %(794/874)가 병의 놓기 단계**다 [→ 정정 2026-09-25 23:58 UTC, R7 35회차 D-1: 798은 머그·쟁반 전도 4편을 포함한 과제 전체 놓기 실패 수; 병만은 384 + 237 + 173 = 794]. 병은 쥔 채 20–24° 기울어(파지점이 위쪽, r2_datagen §4.1) 놓을 때 넘어지거나 굴러 나간다. 쥐기·들기 실패는 과제를 통틀어 29편뿐이다.
...
- 벽시계: 파일럿 11:16:17Z → 마지막 편 21:56:28Z = 10 h 40 min(본 생성 12:00–21:56Z 약 9 h 56 min [→ 정정 2026-09-25 23:58 UTC, R7 35회차 D-2]). 평균 약 560편/h(5프로세스). 프로세스당 편 29.7 s = 기록 20.8 s(시뮬 10.0 s) + prefix 선행 실행 8.9 s(편 평균, `check.json`); 쓰기·검사 약 0.5 s는 별도.
