...
## 변경 기록
- **변경 1 (2026-09-26T11:06Z, gpt-5.2 4판 도중(2판 끝, 결과 봄: 0/2) · gpt-5-mini 호출 전)**: 사용자 목적 정정(원문) "나는 학습할 수 있는 llm크기에서 아스트라를 대체하는게 목적이야", "아니 빨리해줘 4판". 목표를 학습 가능한 크기 **Qwen3.5-27B**로 바꾸고 그 대리 **gpt-5-mini**(`gpt-5-mini-2025-08-07`, 입력 $0.25 / 캐시 $0.025 / 출력 $2 per 1M)를 **같은 설정·같은 4판·같은 순서**로 추가한다(조사 문서 6절). gpt-5.2 4판은 끝까지 돌려 '큰 모델 참고'로 둔다. 상한은 두 대리 합계 **3,000원 한 장부**(바꾸지 않음). 추정 gpt-5-mini 호출당 약 1.5–3원, 4판 ≤ 80호출 ≈ 250원 이하. 판정 규칙(4절)은 gpt-5-mini 4판에 그대로 적용하고, gpt-5.2 판정도 따로 적는다. 대리 영샷 결과 = 미세조정 전 출발점. 출력 `/data/harvest/out/open_vlm_proxy_mini/`, 영상 `/data/harvest/videos/open_vlm_proxy/gpt-5-mini/`. 코드: 가격표 한 줄 + `proxy4.sh` 인자(모델·출력·태그).
- **변경 2 (2026-09-26T11:10Z, 사용자 지시 "노노 미니 돌리지마")**: gpt-5-mini 실행을 standard 0의 7호출(23.3원) 뒤 멈춘다. 이 부분 편은 판정에 쓰지 않는다. gpt-5.2 4편만 결과로 보고한다.
