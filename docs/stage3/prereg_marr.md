...
## 8. 변경 기록
- **변경 1 (2026-09-26 06:48 UTC, 어느 판도 시작 전 — 판정 밖 지표만)**: (a) `run_train.sh c0`에 `--extra-out`을 더해 C0의 청크 MSE(확정·예측 보기)도 같은 사본·파드에서 낸다(5절 '청크 MSE'를 A와 C0 짝으로 비교하기 위함; 예측 기록·판정 입력은 불변) — `run_train.sh` `642f7b0564d61a36`; (b) 5절 보조 궤적 오차의 참고 기준을 내는 `tools/marr_real/trace_ref.py` `b2a03d8736723491`(학습 분할 점 자리별 평균 궤적을 답했을 때 검증 RB2 라벨 행 화소 오차, 같은 공식). 판정 규칙·문턱·관문 불변.
