...
## 변경 기록
- **G0 스모크 통과 (2026-09-26 06:12 UTC, 등록 커밋 9322853 고정 사본)**: 작업자 충돌 0, 두 팔 `eval_runs` 성공, 결합 행·블롭·요약·`dry_*.jsonl`·영상 모두 기록, 흐름 답 1개 유효(Qwen 실제 3.6 s → 붙잡은 7.0 s), 장부 2행 0원. 본 실행 시작 06:14:39 UTC.
- **변경 1 (2026-09-26 06:18 UTC, 본 실행 결과를 보기 전; 판정·문턱 없음이라 지표 추가만)**: 스모크의 결합 팔 프레임·행동 기록에서 **두 층 관문이 VLA 청크의 그리퍼 닫기를 T1 근접 전까지 붙잡다가 풀리는 순간 그리퍼 명령이 한 틱에 약 14 mm 뛰는 것**을 봤다(0.10475 → 0.0908, 0.98–3.30 s 동결). 그래서 `analyze.py`에 **실제로 보낸 명령의 틱당 계단**(그리퍼 |Δw| 최대·5 mm 초과 수, 팔 관절 |Δq| 최대·p99·0.04 rad 초과 수, 그리퍼 동결 최장 시간)을 두 팔 모두에 더한다(Inspect Robots `actions/` 기록에서; 정본 없는 등록 밖 지표로 표시). 또 저크는 100 Hz 틱 경로 그대로(등록한 식)로 보고하되 탐침의 0.83 m/s³(다른 실행기·경로)와 비교하지 않는다. 분석 도구만 바뀌고 실행 코드(고정 사본)는 그대로다 — 분석은 이 변경 커밋의 새 사본으로 돌린다.
