...
마지막 갱신: 2026-09-25 07:40 UTC (R1–R6 완료, R7 순회 중 — 회차별 판정·연속 무결 횟수는 §2.8 마지막 줄, 정본 §1~§75)
...
- **R7 10회차(2026-09-25 07:00 UTC) FAIL**: DEFECT 1(D1 — E-M4의 H = 1이 사전 등록 정의 "같은 스텝을 시각을 당겨 2~3회 묻기"(정본 §2, M4 §4.1, E §2.6·§2.7-4·§2A.3)를 구현하지 않아 스텝당 표 1개·확정 0, 파드 Isaac C5 `commits` 0), DOC 1(D-1 이 문서 9회차 줄·`r7c9_fixes.md:15`의 N4 "[결정 필요]"가 §74 보충 뒤에도 그대로), SCOPED 11, NOTE 10. 보고서 `docs/stage3/results/r7_cycle10.md`. → 수정(2026-09-25 07:40 UTC, `docs/stage3/results/r7c10_fixes.md`, 정본 §75): D1 `m4.early_ask_steps` — H = 1 호출은 시작 시각이 [t_send + d̂, t_send + lead_max]인 모든 스텝의 앞당긴 표(E §2.6 스케줄, E0.5 재생·오프라인 E0 수와 격자 일정에서 같음), `closed --m4-lead-max`(기본 1.0 s, 후보 {1.0, 1.5}는 E0 뒤 메인 확정), H = 3은 그대로; N1 나이 비교 경계(`m4.older_than`, 정확히 1.50·5.00 s 유지); D-1; N4 near "< 5 cm" 옛 서술 표시; N7 논문 갱신 항목; "값 전달"만 본 대조표 행을 행동 시험으로(H 스텝당 표 수·조기 호출 예산·FLIP_TH 끔·in-flight). 무작위 대조 300판: **H = 3 판 155/155 모든 조건 비트 동일**, H = 1은 의도대로 달라짐(확정 있는 판 C5 32 → 88). 파드 Isaac C5 H = 1 `commits` 187. 알려 둘 한계: 단계 A DecCall은 질문 하나라 이 런타임에서 H = 1과 H = 3의 차이는 표 창뿐(판정 7 해석, 정본 §75 **[결정 필요: 메인 세션]**). 시험 로컬 860 passed/11 skipped(841 + 새 15 + 재생 조사 에이전트의 커밋 안 된 시험 4), 파드 CPU 898/3. **코드 변경 → 연속 무결 0회** → 11회차.
