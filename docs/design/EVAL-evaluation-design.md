...
- **(정본 §36~§38) AI Worker와의 정합 — 3층** (Claude 결정, `D21-aiworker-zed.md` §3.7): **AI Worker는 RoboDojo 지원 로봇이 아니다**(원문: 시뮬 "ARX X5 bimanual platform, with the two arm bases separated by 0.6 m", 실물 "ARX X5, Piper, and Piper X", 카메라 "one head camera and two wrist cameras"; 새 로봇 추가 절차에 대한 원문 언급 없음).
  1. **결정 층 비교(H2·H3, 낙폭 RD) = RoboDojo-Sim ARX X5 그대로**(공개 수치 π0.5·Astra·GPT-as-Policy와 같은 열). M1은 트랙 D로 시뮬 깊이를 쓴다. 보조로 **트랙 D-stereo**: 머리 카메라 옆 63 mm에 가상 카메라를 하나 더 두어 스테레오 쌍을 렌더하고 AI Worker와 **같은 Fast-FS/M1 코드 경로**를 쓴다. 표에 "입력 다름"을 표기한다.
  2. **개발·GT·모듈 실험(E0–E3, E-M4, E-R·E-AE) = cyclo_lab AI Worker 한 팔 + `ZED_M` 쌍둥이**, Isaac Sim 5.1 / Isaac Lab 2.3(RoboDojo와 같은 판본). 모델은 FFW-SG2 베이스 고정 우선, 한 팔 설정이 안 되면 FFW-BG2(E §1.4).
...
- Q1 보충(정본 §37): 결정 층 비교는 RoboDojo ARX X5에서 트랙 O/D 선택 규칙 그대로이고, 보조 트랙 D-stereo를 더한다(§2.1).
