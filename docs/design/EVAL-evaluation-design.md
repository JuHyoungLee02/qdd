...
- **문구 규칙 3(D7, 00 §18)**: "처음 잰다"·"최초"·"first" 같은 선점 표현을 쓰지 않는다. 새로움은 이 문장으로만 적는다: "**같은 인식·같은 실행기 위에서 결정 층만 바꿔**, 두 조건의 깨끗한 장면 → 바뀐 환경 낙폭을 짝지어 잰 연구는 없다(2026-09-22 색인 기준)." RoboDawn(N11: C2R Astra 무학습 53.2%·1-shot 73.6% 대 π0.5 46.0%)과 RoboDojo Astra(§4.3: −11.1% 대 −72.2%, 우리 계산)는 반박이 아니라 **이 주장을 뒷받침하는 선행**으로 인용한다. 우리와의 차이(같은 인식 앞단 없음 — 하네스 주석 카메라·cm 상태·탁자 높이·명령당 계획기·판정 요령 프로파일, 낙폭 짝 비교 없음 — clean 점수 없음·0-shot은 재현 구성 미제공 보고치, RoboDojo는 `_random` 미평가, 비정지 아님)를 인용 문장에 같이 적는다(D8, 00 §19). "같은 인식 앞단" 한정어가 필요한 근거: Astra RoboDojo 자체 평가 22.58% 대 RoboDawn 하네스 35.67%(§3.2 항목 2).
