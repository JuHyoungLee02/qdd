...
- 시간 추정은 **미측정**이다. Astra 첫 토큰은 medium 5.9 s, **high(잠정 기본 — 이 프로젝트 적용은 [결정 필요] 5) 약 73 s**, xhigh 약 198 s(plan §1, Artificial Analysis 2026-09-23 값, 날마다 변동). high × 실패 호출 여러 번이면 에피소드가 수 분 이상일 수 있고, 동기 트랙에서는 이 대기가 시뮬 시간으로 드러나지 않는다. B1a형 재현(medium) × 최대 100호출도 10분 이상일 수 있다. S3에서 실측해 S4 규모를 정한다. 원칙 세 개(사용자 원칙 2 + 잠정 기본 1)의 수치 충돌은 M9 §7 [결정 필요].
