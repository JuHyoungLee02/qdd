...
7. **effort**: 우리 시스템과 Astra를 쓰는 모든 기준(B1b, B5b 등)의 Astra effort **기본 = high**(사용자 원칙 "낮추지 않는다", 00 §13). B1a(RoboDojo 원문)의 medium은 **재현 조건으로만** 쓴다(공개 결과 + 같은 layout medium 재실행 일부로 판본 점검). B1a 대 우리 비교 표에는 "effort 다름(medium 대 high)"을 각주로 적는다.
...
- Q2 effort: **정리됨(00 §13)** — 우리 시스템 기본 high(사용자 원칙), B1a의 medium은 재현 조건으로만. 첫 판의 "주 비교는 medium" 권장은 사용자 원칙 반대편에 기운 권장이라 거둔다(D4 D-4). 남는 비용·시간 문제(high 첫 토큰 약 73 s)는 M9 §7 [결정 필요]에 묶는다.
