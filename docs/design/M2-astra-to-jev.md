...
계약 검사기(코드, 게시 전)
1. JSON 스키마·필수 칸(각 단계 `exit` 비지 않음) 검사. 실패하면 Astra에 오류 목록과 함께 1회 재요청(구조화 출력 재시도). 참고: MCP 명세에서 `outputSchema` 준수는 서버 MUST이지만 **클라이언트 검증은 SHOULD(권장)**다(D2 A1). 우리가 검사를 필수로 두는 것은 우리 설계 선택이지 MCP가 요구해서가 아니다.
