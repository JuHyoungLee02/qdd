...
> 개정 2026-09-24 (정본 §31, D18 반영, `D18-confidence-patch.md`, user-log 32 "너가 옳다고 생각하는 것대로 … 근거가 있어야 돼" — Claude 결정): §4.5의 A4·A5를 **A4′ 적응형 `patch | replace`**와 **A5′ "1회 호출 → 코드 검사(실행 기반 전제·효과 검사 포함) → 순차 수리 재호출(최대 2회)"**로 바꿈 — 다수결은 삭제(옛 A5 줄은 "삭제(§31)" 표시로 기록만 남김). §2.5 신설(선행 근거 행), §4.1 검사기 7(실행 기반 전제·효과 검사)과 수리 규칙 갱신 줄(비정지 호출 = 이전 유효 계약 유지 + 백그라운드 수리), §4.2 R6·R7 갱신 줄, E-M2-3 A5′ 기록, **E-M2-4 신설**(재계획 형식 {patch 전용, 이전 계약 넣고 전체 재생성, 적응형} 비교), §6-11, §7-5 해소, §8. 형식은 "선행은 이렇게 했다 → 우리는 이렇게 한다". 사용자 원칙("실패할 때마다 Astra 개입", T0 뒤 웬만하면 안 멈춤, effort 기본 low + low·high 비교)은 그대로다.
> 개정 2026-09-24 (정본 §28, D17 반영, `D17-api-consistency.md`): §2.4 신설 — API 일관성·재계획 고정 근거 행(HIGH/MED만. D17 §2의 LOW 12편은 근거로 쓰지 않음, MED-LOW 보조 4편도 넣지 않음). §4.5 신설 — **Astra 일관성 규약 A1~A6**: A1 템플릿 해시 `astra_prompt_id`, A2 입력 정규화, A3 출력 = 계약 JSON + 검사기 1–6 유지(2턴 변환은 첫 시도 통과율 < 80%일 때만 비교), A4 재계획 고정 = **잠정 유지, D18 조사 결과 전 미확정**(정본 §29. R6 승격: 기본 출력 `patch`, 단계마다 `change_reason`, 이미 실행한 단계 변경 금지, 편집 거리 기록), A5 T0 전용 K = 3 병렬 다수결 = **보류(§29, 대체안 조사 D18 중)** — 사용자 의견(user-log 31)으로 잠정 기본에서 내림, A6 재현 기록. §4.2 R6 승격 표시, E-M2-3 지표·조건 추가, §6-10, §7-5 갱신. A4는 Claude 설계 선택이고 사용자 원칙("실패할 때마다 Astra 개입", T0 뒤 웬만하면 안 멈춤, effort 기본 low + low·high 비교)은 그대로다.
...
### 2.5 계획 확인·재계획 형식 (정본 §31, `D18-confidence-patch.md`)
등급은 D18 조사 기준(user-log 14)을 옮겼다. (abs)는 abs 원문, (요약)은 요약 도구를 거친 인용이라 본문 반영 전 원문 대조가 필요하다(D18 §1). D18 §5의 LOW 10편은 근거로 쓰지 않고, MED-LOW 보조(2609.19654·2609.07395)는 이 표에 넣지 않았다.

| 이름 | 분야 | 어디서 최고였나 (조건) | 신뢰도 | 기간 | 학습 없이 | 로봇 |
|---|---|---|---|---|---|---|
| **Plan Verification for LLM-Based Embodied Agents** (2509.02761) | LLM 체화 계획 검증 | Judge LLM이 계획을 비평하고 Planner LLM이 고치는 반복 수리. (abs) "up to 90% recall and 100% precision", "96.5% of sequences requiring at most three iterations". 조건: TEACh 오프라인 데이터 정제 | MED(UIUC, 학회 없음) | 안(v1 2025-09-02) | 예 | 체화(오프라인) |
| **VerifyLLM** (2507.05118) | 로봇 계획 검증 | 실행 전에 계획을 LTL로 바꿔 빠진 전제 조건을 형식 검사 | MED(IROS 2025 — abs Comments, MIPT·AIRI) | 안(v1 2025-07-07) | 예 | 예 |
| **When Does Verification Pay Off?** (2512.02304) | LLM 검증기 | (abs) "verification across model families is more effective than either self-verification or verification within the same family" | MED(NYU, ICLR 2026 워크숍) | 안(v1 2025-12-02) | 예 | 아니오 |
| JEV-Star (2609.27331, 정본 §24) | 계층 게임 에이전트 | 계획 요청이 실패하거나 늦으면 "the previous valid plan available"로 이전 계획을 유지 | M8 §2 행 그대로 | 안 | 예 | 아니오(게임) |
| **Plan-and-Act** (2503.09572) | LLM 웹 에이전트 | (요약) "After each iteration, the Planner takes in the current state as well as the previous plans and actions and generates a new plan." 정적 29.63% → 동적 재계획 53.94%, WebArena-Lite(요약 도구 경유 수치) | HIGH(ICML 2025 — abs journal-ref, UC Berkeley) | **기간 밖(2025-03-12), 기초 문헌** | 아니오(계획기 학습) | 아니오 |
| π0.5 (2504.16054) | VLA | (요약) "during each step of inference, the model first predicts the semantic subtask" — 추론 스텝마다 하위 과제를 다시 예측 | HIGH | 안 | 아니오 | 예 |
| **SWE-Edit** (2604.26102) | 코드 편집 에이전트 | 찾아 바꾸기가 기본이지만 (abs) "adaptive find-replace/whole-file-rewrite policy improves edit success by 12.5 pp" | MED-HIGH(Microsoft, 학회 표기 없음, 14★) | 안(v1 2026-04-28) | — | 아니오 |
| **AdaEdit** (2604.27296) | 코드 편집 | (abs) "consistently matches the accuracy of full-code generation, while reducing both latency and cost by over 30%". 조건: 긴 코드 편집 | MED-HIGH(Findings ACL 2026, NJU·Alibaba) | 안(v1 2026-04-30) | — | 아니오 |
| AgenticCache (2604.24039, §2.4) | 체화 계획 캐시 | "plan locality" — patch가 이기는 조건(변경이 국소적일 때)의 근거로만 쓴다 | HIGH(MLSys 2026) | 안 | — | 체화 과제 |
| CISC (2502.06233) · DeepConf (2508.15260) | LLM 추론 투표 | 둘 다 majority vote(self-consistency)를 **비교 기준선**으로 둔다. DeepConf (abs) "self-consistency with majority voting … often leads to diminishing returns". 확신 가중에 토큰 확률이 필요해 Astra에는 못 씀 | HIGH(CISC ACL Findings 2025, DeepConf Meta) | CISC 기간 밖(2025-02-10), DeepConf 안 | — | 아니오 |
| **Reasoning Models Better Express Their Confidence** (2505.14489) | 말로 한 확신 | (abs) "achieve strictly better confidence calibration than their non-reasoning counterparts in 33 out of the 36 settings" | HIGH(NeurIPS 2025, KAIST·LG AI) | 안(v1 2025-05-20) | 예 | 아니오 |

patch 전용을 표준으로 쓰는 강한 로봇·에이전트 논문은 찾지 못했다(D18 §3, 검색 범위 안의 결과이며 부재 주장 아님).
...
7. **(정본 §31 A5′) 실행 기반 전제·효과 검사** [접목]: 단계 i의 `entry`(전제 술어)를 현재 측정 상태(i = 첫 미실행 단계) 또는 앞 단계의 예상 효과(`exit`·`expected_after`, i > 첫 미실행 단계)로 코드 평가한다. 연쇄 일관성(앞 단계 효과가 뒤 단계 전제를 채우는지)과 금지 술어(`forbidden`이 어느 단계의 예상 상태에서 참이 되는지)를 함께 본다. 술어 값이 `unknown`이면 실패로 세지 않고 기록만 한다 [가정]. 선행: VerifyLLM(2507.05118, IROS 2025, MED)은 실행 전에 계획의 빠진 전제 조건을 형식 검사했다 → 우리는 같은 검사를 등록부 술어와 측정 상태 위에서 코드로 한다.
...
- **(정본 §31 A5′로 갱신)** 위 두 줄은 A5′가 덮는다. **T0**: 검사 1~7 중 실패가 있으면 위반 목록을 붙여 **순차 수리 재호출**, 최대 2회 [가정](선행 2509.02761의 "≤3회 96.5%"보다 보수적). 통과할 때까지 기다린다(정지 허용 구간). 상한을 넘으면 같은 T0 안에서 새 계획 요청(위 규칙 그대로). **비정지 호출(T_fail·T3 등)**: 검사 실패 시 **이전 유효 계약을 유지**하고(R7 (i)~(iii) 그대로) 수리 재호출은 **백그라운드**로 최대 2회 한다. 로봇은 멈추지 않는다. 수리된 계약이 검사를 통과하면 R3 시점에 교체한다. 선행: JEV-Star(§24)는 계획 요청이 실패하거나 늦으면 "the previous valid plan available"로 이전 계획을 유지했다 → 우리도 같은 모양으로 한다.
...
- **R6 부분 갱신**: 재계획 때 Astra가 전체 계약 대신 `patch`(바꿀 단계만, `parent_version` 명시)를 낼 수 있다. 검사기는 patch 적용 결과를 전체 검사. 목적: Astra 출력 토큰·지연 감소(추론, 측정 필요). (정본 §28 A4) 실행 중 재계획의 **잠정** 출력으로 승격했다(정본 §29: D18 조사 결과 전 미확정) — 입력·`change_reason`·실행한 단계 변경 금지·편집 거리 기록은 §4.5 A4. 전체 계약 재요청은 예외·비교 조건으로 남는다(E-M2-3 추가 판정 (2)).
- **(정본 §31 A4′로 갱신)** R6의 `patch` 기본은 적응형 `mode ∈ {patch, replace}`로 바뀌었다(§4.5 A4′). `replace`도 이전 계약을 입력에 넣고, 이미 실행한 단계는 두 경우 모두 불변이다. "전체 계약 재요청은 예외·비교 조건"은 A4′의 replace 강제 규칙과 E-M2-4로 옮겼다.
...
- **(정본 §31 A5′로 갱신)** R7의 "이 patch를 고치려고 따로 왕복하지는 않는다"는 폐기한다. 거부 (i)~(iii)는 그대로 두고(이전 유효 계약 유지, `C_assume` 신호, 로봇 계속), 위반 목록을 붙인 수리 재호출을 백그라운드로 최대 2회 한다(§4.1 수리 규칙 갱신 줄). 수리가 patch 실패에서 시작되면 수리 호출은 `replace`로 요청한다(A4′ (iii)).
...
### 4.5 Astra 일관성 규약 A1~A6 (정본 §28, D17 — §31로 A4′·A5′ 확정, A5 다수결 삭제)
...
- **A4′ 재계획 형식 = 적응형 `patch | replace`** (정본 §31, Claude 결정 user-log 32; §28 A4 "patch 고정"과 §29 "잠정 유지, 미확정"을 대체):
  - 선행은 이렇게 했다: Plan-and-Act(2503.09572, ICML 2025, UC Berkeley)는 매 스텝 "the Planner takes in the current state as well as the previous plans and actions and generates a new plan"으로 이전 계획을 입력에 넣고 다시 생성해, 동적 재계획으로 WebArena-Lite 29.63% → 53.94%를 얻었다(요약 도구 경유 수치). π0.5(2504.16054)는 추론 스텝마다 하위 과제를 다시 예측한다. 코드 편집에서는 SWE-Edit(2604.26102, Microsoft)이 "adaptive find-replace/whole-file-rewrite policy improves edit success by 12.5 pp"를, AdaEdit(2604.27296, Findings ACL 2026)가 전체 생성과 같은 정확도에 지연·비용 30% 이상 감소를 보고했다. patch 전용을 표준으로 쓰는 강한 로봇·에이전트 논문은 찾지 못했다(D18 §3).
  - 우리는 이렇게 한다: 입력은 §28 A4 그대로(활성 계약 + `based_on` 이후 변한 사실 목록 + 이미 VERIFIED·EXECUTING 단계). 출력 `mode ∈ {patch, replace}`. 기본은 `patch`(변경이 국소적일 때 지연·토큰 이득과 약속 보존 — AgenticCache 2604.24039, MLSys 2026의 "plan locality"). 다음이면 `replace`로 강제한다 [가정 규칙]: (i) 실패가 깨뜨린 전제 술어에 기대는 미실행 단계가 절반 초과, (ii) 목표·물체 역할 변경, (iii) patch가 A5′ 검사를 통과하지 못함. (i)·(ii)는 코드가 호출 전에 판정해 요청에 `mode: replace`를 적고, (iii)은 수리 재호출을 `replace`로 요청한다 [우리 해석]. `replace`도 이전 계약을 입력에 넣는다(Plan-and-Act식). 두 경우 모두 이미 실행한 단계는 바꾸지 않고, 바꾸는 단계마다 `change_reason`(근거 변화 사실 id)과 편집 거리(바뀐 단계 수)를 기록한다. 적용 결과의 검사·거부는 R7 + A5′.
  - 비교: E-M2-4 {patch 전용, 이전 계약 넣고 전체 재생성, 적응형}(지표: 성공률, 출력 토큰, 첫 토큰~완료 지연, 약속 보존율).
  - 계약 요약을 Jev 조각에 계속 싣는 지금 설계(§4.1 조각)는 "periodic plan reminders"(2604.12147, MED-HIGH)와 같은 방향이다. 사용자 원칙("실패할 때마다 Astra 개입")은 그대로 — 부르는 방식이 아니라 받는 형식의 규칙이다.
- **A5 투표 — 삭제(§31)**: §28의 T0 전용 K = 3 병렬 다수결(계획 서명 다수결)은 §29로 보류됐다가 §31로 삭제했다(A5′로 대체). 사용자: "세번 동시에 다수결로 부르는 방식 자체가 너무 오리지널 예전 방식 같은 느낌"(user-log 31). 기록: K = 3과 계획 서명 정의는 설계 선택([가정])이었다. 같은 입력 반복 일치도는 E0 측정 지표로만 남는다.
- **A5′ 계획 확인 = 1회 호출 → 코드 검사 → 순차 수리 재호출** (정본 §31, Claude 결정 user-log 32):
  - 선행은 이렇게 했다: LLM 체화 계획 검증 연구(2509.02761, UIUC, MED)는 Judge가 계획을 비평하고 Planner가 고치는 반복 수리로 "up to 90% recall and 100% precision", "96.5% of sequences requiring at most three iterations"를 보고했다(오프라인 TEACh 정제 조건). VerifyLLM(2507.05118, IROS 2025, MED)은 실행 전에 계획의 빠진 전제 조건을 형식 검사했다. 다른 계열 검증이 자기 검증보다 낫다는 결과도 있다(2512.02304, NYU, ICLR 2026 워크숍, MED: "verification across model families is more effective than either self-verification or verification within the same family"). JEV-Star(§24)는 계획 요청이 실패하거나 늦으면 "the previous valid plan available"로 이전 계획을 유지했다.
  - 우리는 이렇게 한다: Astra를 effort low로 1회(K = 1) 부른다 → 검사기 1–6에 **실행 기반 전제·효과 검사**(§4.1 검사기 7: 단계 i의 전제 술어를 현재 측정 상태 또는 앞 단계 `expected_after`로 코드 평가, 연쇄 일관성, 금지 술어)를 더한다 → 실패하면 위반 목록을 붙여 **순차 수리 재호출**(최대 2회 [가정], 선행의 "≤3회 96.5%"보다 보수적). T0(정지 허용)에서는 통과할 때까지 기다린다. 비정지 호출(T_fail·T3)에서는 검사 실패 시 **이전 유효 계약을 유지**하고 수리는 백그라운드로 한다(로봇은 계속 — 사용자 원칙 불변). 다른 계열 검증(Jev typed 질문)은 E-M2-3 비교 조건으로만 둔다(Jev와 Astra가 다른 계열인지 미확인).
  - 왜 다수결이 아닌가: 같은 모델 병렬 다수결은 2025–26 문헌에서 비교 기준선이다(CISC·DeepConf가 majority vote를 기준선으로 둠). 같은 관측 반복은 증거를 늘리지 않는다(PACT, 정본 §24). 확신 가중 투표는 logprob가 필요해 Astra에 쓸 수 없다.
  - Astra가 말로 한 확신(`astra_conf`, 결정 지점별)은 계약에 **기록만** 한다(추론 모델의 말로 한 확신이 더 잘 보정된다는 결과 2505.14489 NeurIPS 2025 HIGH; 에이전트 수준에서는 이득이 없다는 반대 결과 2609.07395는 MED-LOW라 참고만). 게이트로 쓰지 않는다. 결과와 맞춘 보정 곡선만 E-M8에서 그린다.
  - 호출 쪽 규칙(K = 1, 백그라운드 수리, 기록)은 M8 §4.2.
- **A6 재현 기록**: 요청 원문(이미지 바이트 해시 + 원본), `astra_prompt_id`, 모델 ID, effort, 최대 토큰, 사용량, 첫 토큰·완료 시각, 원응답, 검사기 결과, 계획 서명, patch 편집 거리. (정본 §31) + `mode`(patch/replace)와 강제 사유 (i)~(iii), 수리 재호출 횟수(`repair_idx`), 검사기별 실패 목록(1–7), 수리 전후 계약 판본, 비정지 호출의 이전 유효 계약 유지 시간, `astra_conf`(기록만).
...
- (정본 §28) 추가 측정: patch 편집 거리 분포, `change_reason` 없는 변경 수, 이미 실행한 단계를 바꾸려 한 patch 수, (A5는 보류 — 정본 §29) 계획 서명 일치율은 E0 같은 입력 반복에서만 잰다, `astra_prompt_id`·모델 ID별로 나눈 통과율. 추가 조건: 첫 시도 통과율 < 80%일 때만 "자유 계획 → 2턴 스키마 변환"을 비교 조건으로 넣는다(A3). Astra 조건 사이 비교는 반복 실행 수와 검정력 계산을 사전 등록한다(2%p 검출에 약 9회 기준, 정본 §28). 단일 실행 비교로 결론 내지 않는다.
- (정본 §31 A5′) 추가 기록·측정: 호출마다 수리 재호출 횟수(0/1/2, 상한 도달 여부), 검사기별 실패(1–7, 특히 7의 전제·연쇄·금지 위반 유형), 수리 뒤 통과율(1회·2회 뒤), 비정지 호출에서 이전 유효 계약을 유지한 시간(백그라운드 수리 완료까지), 수리 실패로 끝난 비율. `astra_conf`는 기록만 하고 E-M8에서 결과와 맞춘다. 비교 조건(선택): **다른 계열 검증** — 검사를 통과한 계약을 Jev typed 질문(단계별 "이 단계의 전제가 앞 단계 뒤에 성립하는가")으로 한 번 더 검증한다(2512.02304, MED). 판정([제안], 실행 전 고정): T0에서 2회 수리 뒤에도 불통과가 10%를 넘으면 상한 3회(선행 값) 조건을 더한다.

### E-M2-4 재계획 형식 (정본 §31 A4′, 시뮬 폐루프 + 기록 재생)
- 조건: {P patch 전용(§28 A4), R 이전 계약 넣고 전체 재생성(Plan-and-Act식), A 적응형(A4′ 기본)}. 세 조건 모두 입력(활성 계약 + 변한 사실 + 실행한 단계), `astra_prompt_id` 계열(출력 형식 지시만 다름), effort {low, high}, 실행한 단계 불변 규칙이 같다.
- 지표: 성공률, Astra 출력 토큰, 첫 토큰~완료 지연, 약속 보존율(재계획 전후 미실행 단계 중 바뀌지 않은 비율), 편집 거리, A5′ 검사 첫 시도 통과율, A에서 `replace` 강제 비율과 사유 (i)~(iii)별 분포.
- 반복: Astra 조건이라 반복 실행 수와 검정력을 사전 등록한다(2%p 검출에 약 9회, 정본 §28). 단일 실행 비교로 결론 내지 않는다.
- 판정([제안], 실행 전 고정): (1) A의 성공률이 R보다 낮지 않고(짝 부트스트랩 차이 95% 하한 > −2%p) 출력 토큰 또는 완료 지연이 R보다 20% 이상 적으면 A 유지. (2) R이 A보다 성공률에서 유의하게 높으면(하한 > 0) 기본을 R로 바꾸는 것을 [결정 필요: 메인 세션]으로 올린다. (3) P가 A와 성공률이 같고(차이 95% 구간이 ±2%p 안) 지연이 더 짧으면 replace 강제 규칙 (i)~(iii)의 문턱을 다시 본다.
...
10. **A4·A5 근거의 한계** (정본 §28, §29로 A5 보류·A4 미확정): AgenticCache·APC는 계획 캐시·템플릿 재사용 연구라 "patch 고정이 계획 품질을 올린다"는 직접 근거가 아니다 [우리 판단]. patch 고정은 첫 계획의 잘못을 끌고 갈 수 있다(바꾸려면 `change_reason`이 필요하므로, [우리 추론]) → E-M2-2·E-M2-3에서 편집 거리와 성공률을 함께 본다. K = 3, 계획 서명 정의는 논문 값이 아니라 설계 선택([가정])이었고, A5는 사용자 의견으로 보류됐다(D18이 대체안 조사). Astra에 seed·응답 식별 필드·prompt caching이 있는지는 확인하지 못했다(D17 §6) — 어느 경우든 temperature 0이나 seed로 결정성을 얻을 수 없다는 전제는 유지한다.
11. **A5′·A4′ 근거의 한계** (정본 §31): 2509.02761의 "≤3회 96.5%"는 오프라인 TEACh 정제 조건이지 온라인 로봇 재계획이 아니다(MED, 학회 없음). 2512.02304는 워크숍(MED)이고 Jev와 Astra가 다른 계열인지 확인하지 못했다. Plan-and-Act 수치(29.63% → 53.94%)는 요약 도구 경유이고 웹 과제다. SWE-Edit·AdaEdit는 코드 편집이지 로봇 계획이 아니다. replace 강제 규칙 (i)~(iii)과 수리 상한 2회는 [가정]이다. A5′ 검사에는 통계적 보장이 없다 — logprob가 없는 Astra에서는 외부 검사로 "틀린 것을 잡는" 구조가 한계다(D18 §6). 국소 편집이 전체 재계획보다 성공률이 낮았다는 결과(2609.19654, 여행 도메인)는 MED-LOW 보조라 근거로 쓰지 않지만 E-M2-4로 확인한다.
...
5. 해소(정본 §31 A4′, Claude 결정 user-log 32): 재계획 형식 = 적응형 `patch | replace`(기본 patch, (i)~(iii)이면 replace, replace도 이전 계약 입력, 실행한 단계 불변). §28 A4 "patch 고정"(잠정)은 대체됐다. E-M2-4에서 {patch 전용, 전체 재생성, 적응형}을 비교한다. 사용자 원칙은 바뀌지 않는다.
...
8. 해소(00-interfaces §14-4, D4 §14-4): 계약 정적 검사 실패 시 수리 왕복은 **T0에서만** 허용한다. 실행 중 재계획 patch는 검사만 하고, 실패하면 거부 + M7 신호를 보내며 로봇은 멈추지 않는다(R7). `forbidden`·순서 제약은 DFA로 컴파일해 H5·`C_assume` 감시를 통일한다(§4.4). 메인 세션의 잠정 결정(기술 선택)이며 E-M2-3으로 검증한다. `contract_patch_rejected`는 `C_assume`로 센다(00 §15). DFA 형식 검증은 기간 밖 기초 문헌(명세 패턴·런타임 검증)에 기대므로 [결정 필요] 4 승인 전까지 잠정 기본이다(00 §16).
   - (정본 §31 A5′로 갱신) 비정지 호출도 이전 유효 계약을 유지한 채 백그라운드 수리 재호출(최대 2회)을 한다(§4.1 수리 규칙 갱신 줄, R7 갱신 줄). "수리 왕복은 T0에서만"은 T0에서만 기다린다는 뜻으로 좁힌다.
...
- (정본 §31, D18) 2509.02761(학회 없음)·2512.02304(워크숍)는 MED다. Plan-and-Act 29.63% → 53.94%와 π0.5 인용문은 요약 도구 경유라 원문 대조가 남았다. SWE-Edit의 학회는 abs에 없다. 수리 상한 2회가 온라인 로봇 재계획에서 충분한지는 E-M2-3으로 잰다.
