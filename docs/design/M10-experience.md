...
표기: **[원문]** / **[접목]** = 우리 접목안 / [사용자] / [제안] / [결정 필요]. 전제: M1 후보 1(코드 술어) 가정.
...
| 항목 | 내용 |
|---|---|
| 입력 | 에피소드 기록: 세션 계약, 결정 스텝 기록(M3 `DecisionStep`: 질문·보기·확률·예상 결과 술어), M4(b) 범주(OK/LAG/DEVIATE/CONTRADICT), M7 사건(FAIL/WARN, 단계 원장 `done_k`/`inv_k`), M9 복구 시도와 결과, 스킬 결과 술어(M6 `exit`), 측정 술어 시계열, Astra 판단문 |
...
| **GEPA** (2507.19457) ○ | LLM 프롬프트 최적화 | 궤적을 자연어로 반성해 **프롬프트 규칙을 진화**, 자기 시도들의 **Pareto 전선**에서 보완 교훈을 합침. GRPO보다 평균 +6%, 최대 +20%, 롤아웃 최대 35배 적음. MIPROv2보다 10% 넘게(초록) | **HIGH(ICLR 2026 Oral)** | 2025-07 | 예 | 확인 못 함 |
...
| **Proactive Memory Agent** (2607.08716) ◎ | LLM 에이전트 | **별도 메모리 에이전트**가 고정 주기로 구조화 메모리를 갱신하고 **"알림을 넣을지, 침묵할지"** 결정. Terminal-Bench 2.0 +8.3pp, τ²-Bench +6.8pp(pass@1). 절제(τ², 행동 에이전트 Sonnet 4.5, 메모리 에이전트 Opus 4.6): 선택적 개입이 수동 노출·항상 주입·조언자형·일반 검색보다 나음. "always-on injection can be competitive but less balanced" | MED(Meta AI, 심사 전) | 2026-07 | 예(주 결과는 API 모델) | 아니오 |
...
| **BATON** 전이 인지 메모리 (2608.16889) ◎ | 로봇 | **"Edges are keyed by transition type rather than by task"**("좁은 입구 용기에 놓기"가 바구니→통으로 전이), 계약 위반을 되써서 해당 전략을 그 전이에서 배제. RoboMemArena 과제 +11.6%, 누적 +14.9%(초록) | LOW-MED(USC, 심사 전) | 2026-08 | 예 | 예 |
| **PragmaBot** (2507.16713) ◎ | 로봇(실물) | VLM이 결과를 시각 판정·반성 → STM(과제 중) → 과제 뒤 LTM 요약 → RAG. STM 35→84%, **LTM 12개 실물 시나리오(8개 처음 봄) 단일 시도 22→80%, RAG > 통째 프롬프트** | MED-HIGH(**RA-L 채택**, ETH Hutter) | 2025-07 | 예 | 예 |
...
| 이름 | 어디서 최고였나 (조건) | 신뢰도 |
|---|---|---|
| **Experience-following** (2505.16067) ◎ | 입력이 비슷한 기억을 꺼내면 출력도 비슷해진다(experience-following) → **오류 전파**, **어긋난 경험 재생**. 표 1(4개 에이전트, 열 순서 RegAgent / EHRAgent / AgentDriver / CIC-IoT): 고정 메모리 67.53 / 16.75 / 40.11 / 71.50, **전부 추가 55.48 / 13.05 / 32.32 / 59.90(고정보다 나쁨)**, 엄격 선별 추가(사람 판정) **70.95 / 38.50 / 51.00 / 85.40**, 자동 판정 C1~C3은 그 사이. 표 2: **이력 기반 삭제**(꺼냈을 때 뒤 과제가 실패한 기록 삭제)가 엄격 판정과 짝지을 때 대개 최고, 결합 삭제가 메모리를 가장 줄임. "future task evaluations can serve as free quality labels" | **HIGH(ACL 2026 long, aclanthology 2026.acl-long.27 — 검색 결과로 확인, 페이지 본문 미열람)**, Harvard·MSU |
| 2606.15017 표 10 △ | ReasoningBank LLM 판정이 "성공"이라 한 것 중 52.9%(Gemini)·59.5%(GPT) Shopping이 실제 실패 | HIGH(EMNLP 2026) |
| PragmaBot ◎ | VLM 자기 판정 라벨로도 실물 22→80%(반대 측면: LLM 판정이 쓸모없지 않음) | MED-HIGH(RA-L) |
...
| **2604.27003** ◎ | ALFWorld·BabyAI 순차 과제, ReMe 메모리 모듈, 고정 백본: **추상 절차 교훈(Insight)은 양의 전이, 원시 궤적(Raw)은 A→B에서 음의 전이**. 음의 전이는 **"hard cases"(메모리 없음 기준 실패 부분집합)에 집중**. 더 세밀한 단위·더 잦은 검색이 항상 좋지 않음, 새 과제 적응이 좋은 설계가 이전 과제 망각(BWT<0)을 일으킴. 오염 원인 3종: retrieval pollution, context competition, memory dilution | LOW-MED("Working in progress") |
...
| SkillsBench v4 △ | 사전 자기 생성 스킬 −8.1~−11.5pp | HIGH(반응) |
| 2511.21730 ○ | 절차 검색에서 **임베딩은 처음 보는 어휘에서 급락**, LLM이 만든 절차 추상은 전이 | LOW(반응 미측정) |
| PragmaBot ◎ | 실물 반복 과제에서 LTM이 크게 도움. 실패 사례: 꺼낸 기억(가림 치우기)을 VLM이 무시해 실패 | MED-HIGH |
...
|---|---|---|
| ExpRAG | (입력, 출력, 피드백) 템플릿, top-k | Astra 사례 단위 = (과제, 단계 k, 스킬 id, 결정 지점 id, 술어 스냅샷, 물체 범주) → (Astra 계획/복구 행동) → (**센서 라벨** + 결과 술어) |
...
| 2505.16067 | 엄격 선별 추가 > 자동 판정 > 전부 추가, 이력 기반 삭제 | 추가 = 센서 라벨 (a)등급만(엄격 판정의 대리). 삭제 = 그 항목을 꺼낸 뒤 해당 결정 지점·단계가 **센서로 실패**한 횟수가 성공 횟수를 넘으면 퇴출(이력 기반 삭제의 로봇판) |
| PhyAgentOS | provenance·scope 붙여 주입, 사전조건 맞을 때만 전이 | Jev 규칙의 **정확 일치 키**와 같은 원리. Astra 교훈도 scope 불일치면 넣지 않음 |
| BATON | 메모리 키 = 과제가 아니라 **전이 유형**, 위반 시 그 전략 배제 | 검색 키 1순위 = (스킬 id, 결정 지점 id 또는 전이 유형 `handoff:pick→place_narrow`). 위반 기록은 "AVOID option" 규칙으로 컴파일 |
| Proactive Memory Agent | 메모리 에이전트가 알림 넣을지·침묵할지 결정, 선택적 개입이 최선 | Astra 호출 때는 **Astra 스스로**(또는 코드 조건) 넣을지 결정 — 별도 에이전트는 호출 비용이 커서 안 쓴다. Jev에는 "키 일치 없으면 침묵"이 같은 원리 |
| MemCompiler / ExpWeaver | 작은 실행기엔 상태 조건 공개, 필요 시만 | Jev에는 교훈 문장 금지, 컴파일 규칙 0~2줄만(plan M10 유지) |
| GEPA | 반성으로 규칙 진화 + Pareto 전선 선택 | **규칙 채택 게이트**: 후보 규칙은 개발 시드들에서 "켠 것 대 끈 것"을 재생 비교, 어느 시드에서도 성공률을 떨어뜨리지 않는(Pareto) 것만 채택. 원문은 프롬프트 최적화용, 로봇 적용 확인 못 함 |
...
```
 ...
 에피소드 뒤 (오프라인)
   ...
   → ④ 컴파일(Astra: 교훈 → typed 규칙 후보) → ⑤ 게이트(재생 + 새 시드, Pareto) → ⑥ 규칙표 반영
   → ⑦ 이력 기반 삭제(counters)
```
...
```
hint: at dp.approach_dir if next_skill=place_narrow and top_clear=yes -> prefer side_front
hint: at dp.release if contact_under=no -> avoid release_now
```
...
| 단계 | 내용 | 출처 |
|---|---|---|
| L1 결정 스텝 라벨 | 각 확정 스텝의 M4(b) 범주: 선택 보기의 `expected_after` 대 측정 술어 → OK / LAG / DEVIATE / CONTRADICT | M4 문서 |
| L2 단계 라벨 | 스킬 단계별 `stop`·`exit` 술어 참/거짓, budget 초과, `inv` 위반(M7 하드 FAIL) | M6 계약, M7 |
| L3 에피소드 라벨 | 과제 수용 기준(단계 원장 마지막 `done_k`)을 **센서 술어로** | PhyAgentOS 수용 기준 |
| L4 책임 단계 | First Missing Milestone(Zetta §2.3)으로 첫 누락 단계 → 그 단계의 결정 지점들에 실패 귀속. BATON식으로 "진입 상태가 틀렸나(앞 단계 책임) / 스킬이 약했나"를 `entry` 술어로 구분 | Zetta, BATON |
| 등급 | (a) 술어로 확정 → 교훈·규칙 재료 / (b) 술어가 못 다뤄 Astra 다중 프레임 판정 → "미검증", Astra 참고만 / (c) 판정 불가 → 저장 안 함 | v3/03 §3.4 |
...
### 4.3 검색 키와 예산
- Astra 키 계층: ① (skill_id, dp_id | transition_type, 실패 유형 술어) 정확 → ② + object_cat → ③ (선택) 임베딩(2511.21730: 처음 보는 어휘에서 임베딩 급락이라 마지막 수단). 최대 top-3 교훈 + 사례 2건, 상한 토큰 T_mem(예: 1,500) [제안 값].
- Jev 키: (skill_id, dp_id, 규칙 조건 술어 전부) 정확 일치만. 부분 일치 없음.
- [결정 필요] Astra 주입 방식: (i) top-k 기본 주입 / (ii) 도구로 필요할 때 조회(ExpWeaver: 강한 모델은 거의 안 꺼냄 → Astra도 안 꺼낼 위험) / (iii) 코드가 "키 ① 일치 있을 때만" 주입(Proactive Memory Agent의 "침묵" 원리를 코드로). 1순위 제안 (iii), 비교는 E-M10-2.

### 4.4 대안
- **대안 A (GEPA식 템플릿 진화)**: 규칙을 따로 붙이지 않고, Jev 질문 템플릿 문구·보기 이름 자체를 오프라인으로 진화(Pareto 게이트). Jev #8(문구 바뀌면 확률 비교 불가) 때문에 **템플릿 교체는 버전 단위로만**.
...
- 같은 M1·M6 계약·Jev 버전·Astra effort(고정, effort 낮추기 금지 규칙 따름).
...
- **Experience-following**(2505.16067, ACL 2026): 비슷한 입력의 기억을 따라 한다 → 틀린 기억 하나가 반복 전파. 이력 기반 삭제로 막지만 삭제 기준이 늦게 작동하면 초기 에피소드가 오염된다.
- **LLM 판정 라벨도 실물에서 통했다**(PragmaBot RA-L, 22→80%). 센서 라벨 원칙의 절대 근거는 아니다. 센서 술어가 못 다루는 과제(변형 물체)에선 (b)등급이 늘어 메모리 재료가 줄어든다.
...
- 선행: 경험 → typed 규칙 컴파일(Kintsugi, AutoRefine, MemCompiler 등, v3/07). 새로움은 **확률 결정 모델(Jev)의 결정 지점에 정확 일치로 붙이고 Pareto 게이트로 채택**하는 부분뿐(좁음, plan §4-5).
- GEPA·Training-Free GRPO·Proactive Memory Agent는 로봇 미적용(확인한 범위). BATON·2604.27003은 심사 전.
...
## 8. 확인 못 한 것
- 2505.16067의 ACL 2026 채택은 ACL Anthology 검색 결과(2026.acl-long.27) 제목으로만 확인, 페이지는 열지 않았다. 표 1의 열별 지표 종류(SR/ACC)는 표 머리 일부만 읽었다.
- 2607.08716 표 1·2의 셀 수치(초록 +8.3pp, +6.8pp만 인용).
