...
| W | 유예 창(호출 수) | 1 (경계 직후 2) | {0, 1, 2}; FLy W. **M5 L1의 "겹친 호출 2개 연속 같은 새 보기"는 W=2 설정으로 흡수**(00-interfaces §5). [정정 R7 7회차, 정본 §72: §3 #3·§4.2 정의로는 W = 첫 도전 표 뒤에 더 필요한 같은 도전 표 수 → 도전 표 2개 = **W=1**, W=2 = 3개, W=0 = 첫 도전 표에서 즉시 교체(코드 `runtime/m4.py`)] 보기 교체 규칙은 여기 한 곳뿐 |
