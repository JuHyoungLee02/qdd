...
> 개정 2026-09-24 07:39 UTC (정본 §36~§39, D21 반영, `D21-aiworker-zed.md`, user-log 37~39 — 로봇·카메라·실물 SG2·힘 입력은 사용자 결정, 깊이·격자·장면·정합 세부는 Claude 결정): §4.1.4 R 입력 아래에 **(정본 §37·§39) 힘 입력 = 관절 전류** 줄 — AI Worker에 손목 F/T가 없으므로 "힘/토크" → **관절 `effort`(전류)와 그리퍼 전류**, 손목 F/T는 추가하지 않음(§39, user-log 39 "에이아이워커 기본으로"), CR-DAgger 근거는 한 단계 약해짐, 절제 조건 **R-noforce**. §4.1.5 신설 — **AI Worker 양팔 스킬**: 기존 pick/place FSM·`arms: left|right|both` 결정 지점 유지, 추가 `look_at`(머리 2축)·`set_lift`(0–500 mm)·`handover`(좌→우)·`bimanual_hold`, 이동 베이스는 범위 밖, 실물 = FFW-SG2 베이스 고정(§38, user-log 38 "Sg2야"). 결정 층은 공유, 스킬·M5 한계·R은 로봇별. §7-13.
> 개정 2026-09-24 07:18 UTC (정본 §34·§35, D20 반영, `D20-expert-role.md`, user-log 35·36 — D35·D36은 사용자 결정, R 설계는 Claude 결정): §4.1.4를 고쳤다 — **주 시스템의 phase 실행은 스크립트 스킬 S**(D36 = S). action expert의 주 역할은 S 명령 위의 **구간 제한 잔차 R**(이미지 없는 입력, Jev가 고른 방향 d̂·크기 구간 [m_lo, m_hi]를 코드 투영으로 지킴, 접근·접촉 근처에서만 켬). 보조 역할은 그림자 예측기(M7 소프트 채널 후보, 내부 절제만). 학습 복구는 R에 흡수(복구 동작 실행에도 같은 제한), 보기 후보 생성은 제외. 학습 실행기 B(§33)는 **E-AE-2 비교 표로만** 남는다. 도메인 무작위화 = 넣는다(D35, 모든 학습 조건에 같은 데이터). §7-12.
> 개정 2026-09-24 06:36 UTC (정본 §32·§33, D19 반영, `D19-action-expert.md`, user-log 33·34 — 사용자 결정 B, 세부는 Claude 설계): §4.1.4 신설 — **phase 실행을 학습 실행기 B가 맡는다**(모든 스킬의 모든 phase, 결정 층 Astra·Jev는 학습하지 않고 바깥). 스크립트 스킬은 **GT 생성기 + 백업 실행기(폴백)**. FSM 전이·스킬 계약·진단 필드는 **센서 측정으로 코드가 계산**(그대로). 폴백은 FAIL이 아님. §7-11 해소.
> 개정 2026-09-24 (정본 §31, D18 반영, `D18-confidence-patch.md`, user-log 32 — Claude 결정): §4.1.2 Jev 일관성 규약 아래에 **J5 Jev conformal 게이트**(E1 뒤 켤 후보) — `question_id@vN`별 보정 집합으로 문턱을 잡아, 예측 집합이 원소 하나면 M4 확정 후보, 둘 이상이거나 `NONE_ESCALATE`를 포함하면 확정 보류(직전 확정 행동 유지 + 감속, 정지 아님), 같은 결정 지점에서 반복되면 Astra로 올림(M8 `T_j5`). 보장은 단계별 주변 커버리지(에피소드 수준 아님), 교환 가능성은 판본 묶기(J1) + 매일 카나리로 지킨다. E1 전에는 게이트 끔(00 §6) 그대로.
> 개정 2026-09-24 (정본 §28, D17 반영, `D17-api-consistency.md`): §4.1.2 보기 이름 규칙 아래에 **Jev 일관성 규약 J1~J4**(M3 §4.1과 같은 규약) — J1 결정 지점 질문마다 `question_id@vN` = 해시(문구 + `option_key` 표준 순서 + 보기 설명 + 표시 문자열(공백 포함) + legend + 상태 직렬화기 판본, enum 설명·순서를 고치면 판본이 오름), J2 상태 직렬화 정규화(M1), J3 M4 겹침 확인의 합의 표는 시간차 상태에서만(같은 입력 되묻기는 측정용, `FLIP_TH`는 E0.5 (ii) 바닥 위에서), J4 게이트·감시는 `question_id@vN`별로만(판본이 바뀌면 게이트 끄고 재보정). §7-10.
...
전제: M1 변환 방법은 기본 = 후보 A(Claude 결정, 정본 §26, B·C는 E3 비교)다. 이 문서는 특정 M1 후보(A/B/C)를 가정하지 않고 **M1 술어 등록부 인터페이스(M1 §4.0)에만 의존**한다(00-interfaces §11.3). 스킬 계약의 모든 술어는 등록부 이름이고 등급(T1/T2/T3)은 등록부가 정한다.
...
- **(00 §24, D12) 보기 이름 규칙 — [결정 필요] D32**: 위 표의 보기(`close_now`, `release_now`, `side_front` 등)는 뜻을 이름에 싣는다. Type-Safe Is Not Error-Free(2609.26758, 09-22, D12 메인 재확인): "renaming the two options from 0/1 to no/yes changes 70.4 more answers per hundred ... shifts AUC from .94 to .23", "The hosted model exhibits the same behavior: the swap changes AUC from .8146 to .5806 and produces 24x as many answer flips as its test-retest floor", "the type-error rate remains 0%". 권고: "Use neutral option identifiers and carry the meaning in the rubric". 결정 지점 보기를 중립 식별자로 바꾸고 뜻은 루브릭(질문 템플릿)에 적을지는 M3와 한 항목으로 사용자가 정한다(SUMMARY §5.1 D32). 결정 지점 id(`dp.*`)는 스킬 고정 id 규칙 그대로이고, 바뀔 수 있는 것은 보기 이름뿐이다. 결정 전까지 위 표는 바꾸지 않는다. 자료는 E0.5 (i)·(ii)(E 문서 §2A). → (정본 §27) **해소**: 아래 규칙.
  ...
  - **(정본 §28, D17) Jev 일관성 규약 J1~J4**(정의 정본 M3 §4.1):
    - J1 결정 지점 질문마다 `question_id@vN` = 해시(문구 + `option_key` 표준 순서 + 보기 설명 + 표시 문자열(공백 포함) + legend + 상태 직렬화기 판본). 위 표의 고정 문구(원칙 (4))와 보기 설명·표시 문자열(공백·구두점 포함)이 모두 판본에 들어간다. 스킬 inputSchema enum의 설명(docstring)이나 순서를 고치면 판본이 올라간다. `jev_choice` 기록(R5)에 `question_id@vN`을 함께 적는다.
    - J2 상태 직렬화 정규화: 같은 세계 상태 → 바이트까지 같은 텍스트(M1 후보 A 표, id·등록부 순, 고정 구간 숫자, 나이 범주).
    - J3 M4 겹침 확인(위 표 "같은 질문 반복")의 합의 표는 시간차 상태에서만 모은다. 같은 입력 되묻기는 측정용이며 합의 표로 세지 않는다. `FLIP_TH`와 LA-2 불일치 해석은 E0.5 (ii)에서 잰 바닥 위에서 한다(두 갈래에서 바닥 p면 잡음만으로 두 호출이 어긋날 확률 약 2p(1−p), 우리 계산).
    - J4 확률 게이트(원칙 (5), E1 뒤)와 감시는 `question_id@vN`별로만(질문 사이 확률 비교 금지, Jev 공식 약점). 판본이 바뀌면 그 결정 지점의 게이트를 끄고 E1 축소판으로 재보정한다.
    - **J5 conformal 게이트 (정본 §31, E1 뒤 후보, 정의 M3 §4.1 J5)**: 선행은 이렇게 했다 — KnowNo(2307.01928, 기간 밖 기초 문헌)·CoFineLLM(2511.06575, MED)은 예측 집합이 하나면 실행하고 아니면 도움을 요청했다. 우리는 이렇게 한다: 결정 지점(`dp_id`) 질문마다 `question_id@vN`별로 보정한다. 예측 집합이 원소 하나면 M4 확정 후보, 둘 이상이거나 `NONE_ESCALATE`를 포함하면 보류(직전 확정 행동 유지 + 감속), 같은 결정 지점에서 반복되면 Astra로 올린다(M8 `T_j5`). `effect: irreversible` 결정 지점(`dp.release` 등)은 원칙대로 M4 확정 + 코드 안전 술어가 필요하고, J5를 켜면 집합 원소 하나가 조건으로 하나 더 붙는다. 보장은 결정 지점 단위 주변 커버리지다(스킬·에피소드 수준 아님). 보정 표본은 결정 지점마다 따로 필요하므로, 드문 결정 지점(`dp.critic_accept` 등)은 표본이 [가정] 크기에 못 미치면 게이트를 끈 채 둔다 [우리 판단].
...
#### 4.1.4 phase 실행 = 스크립트 스킬 S + 구간 제한 잔차 R (정본 §34·§35, `D20-expert-role.md`) — 학습 실행기 B는 비교 조건만(정본 §33)
- **바뀐 것(정본 §34·§35)**: 주 시스템의 phase 실행은 **스크립트 스킬 S**가 맡는다(user-log 35 "무작위, 스킬로해도 좋긴한데 엑션 익스퍼트가 역할을 부가할방법" → D36 = S, D35 = 무작위화 넣음). action expert의 주 역할은 S가 낸 명령 위에 작은 병진 잔차를 더하는 **구간 제한 잔차 R**이다(Claude 결정, 근거 D20). Jev는 방향과 크기 구간을 정하는 층이고(user-log 36 "Jev는 앞으로, 뒤로 좌우 몇 센티를 판단하는 역할임 알지?"), 그 권위는 모델 밖 **코드 투영**으로 지킨다. **결정 층(Astra 계획, Jev 선택)은 학습하지 않는다**(정본 §32 그대로). R은 끄고 켤 수 있는 실행기 부품이라 R을 끄면 S 그대로다. FSM 전이·스킬 계약·진단 필드는 코드 그대로다(아래 B 절의 "바뀌지 않는 것" 줄은 S·S+R에도 같다). R은 전이를 정하지 않는다.
- **R 입력 [제안]**: `skill_id`·`phase_id`, Jev 확정 방향 d̂·크기 구간 [m_lo, m_hi](중심 m_c), S 명령 `a_S(t)`, M1 대상 자세와 불확실도, 관절·그리퍼 폭·힘/토크, 짧은 이력. **이미지는 넣지 않는다**(R-state) — 외관 변화는 M1을 거쳐서만 들어가므로 S와 같은 경로다. crop을 넣는 R-crop은 절제 조건이다.
- **(정본 §37·§39) R의 힘 입력 = 관절 `effort`(전류) + 그리퍼 전류**: AI Worker에는 **손목 힘/토크 센서가 없다**(사양·URDF 어디에도 없음; ros2_control 상태 인터페이스는 `position`·`velocity`·`effort`이고 `effort`는 서보 레지스터 "Present Current"를 읽는다). 그래서 위 줄의 "힘/토크"는 관절 `effort`와 그리퍼 전류로 바꾼다. 손목 F/T는 추가하지 않는다(정본 §39, user-log 39 "에이아이워커 기본으로" — AI Worker 기본 구성). CR-DAgger 근거("both the delta correction data and the force data are crucial")는 손목 F/T 기준이라 **근거가 한 단계 약해진다**. 절제 조건 **R-noforce**(힘 입력 뺌)를 둔다(E-R1). 시뮬에서는 관절 토크에 전류 잡음 모델을 씌운다 [가정].
- **R 출력과 코드 투영 [제안, 투영 식은 우리 접목]**: 출력은 병진 잔차 r̃ = (r̃_∥, r̃_⊥)(회전은 기본으로 없음). S의 누적 명령 이동의 d̂ 성분을 s_S(t) = d̂ᵀ(p_S(t) − p_0)라 하면 실제 명령은 p(t) = p_S(t) + g(t)·Π(r̃(t)), g(t) = 1[near ∨ contact]이다. Π는 모델 밖 코드로, (i) d̂ 성분 r_∥ = clip(d̂ᵀr̃, ℓ(t) − s_S(t), m_hi − s_S(t)) — 진행 중 ℓ = 0(방향 반전 불가), phase 끝 목표에서 ℓ = m_lo(누적 이동의 d̂ 성분 ∈ [m_lo, m_hi]) — (ii) 수직 성분 r_⊥ = (I − d̂d̂ᵀ)r̃를 |r_⊥| ≤ ε_⊥로 자른다. ε_⊥는 standard + 무작위화에서 잰 M1 위치 오차의 conformal 분위 [가정]. 마지막 클램프는 M5 L3 Ruckig이다(M5 §4). near = 목표 5 cm 이내, contact = contact 술어 참(설정 표, M3·M5·M7과 공유). 그 밖에서는 r = 0.
- **기록**: `jev_choice`(권위값), ∫r, 포화 비율, R on/off. M4 (b)의 `ref(t)` = 투영 뒤 실제 명령 p(t), `expected_after`는 그대로(M4 §0).
- **데이터 [접목]**: 라벨 = 특권 시뮬 상태로 돌린 S 행동 − M1 추정 상태로 돌린 S 행동(투영 후). standard + 도메인 무작위화(D35) + 섭동 + DAgger. random 시험 장면은 학습에 쓰지 않는다. 선택 2단계로 T1 phase-exit 술어를 희소 보상으로 한 잔차 RL(ResFiT·PLD식) [제안].
- **근거(D20, 등급 그대로)**: 잔차는 베이스가 무엇이든 붙는다 — 손으로 설계한 제어기 위(Residual Policy Learning 1812.06298, 기간 밖 기초 문헌: "RPL consistently and substantially improves on the initial controllers"), BC 베이스 위(ResFiT 2509.19301, MED-HIGH: 실물 휴머노이드 "boosted the performance of the base model from 14% to 64%"), 물체 상태 입력 잔차(Object-Centric Residual RL 2606.18953, MED: sim→real "improves the success rate from 42% to 76% zero-shot", 물체 자세 76/100 대 이미지 47/100, "the visual domain gap is the dominant sim-to-real barrier"), 힘 입력 잔차(CR-DAgger 2506.16685, NeurIPS 2025, HIGH: "both the delta correction data and the force data are crucial"). 잔차는 제한해야 한다(Policy Decorator 2412.13630, 기간 밖 기초 문헌·ICLR 2025: "Bounded residual action is essential"; PLD 2511.00091, ICLR 2026, HIGH: "bounded within a range of [−ξ, ξ]"). 원문의 제한은 학습 안정성용이고, Jev 권위 보호에 쓰는 것은 우리 접목이다. 스크립트 베이스 위 2025–26 잔차 결과는 찾지 못했다(기초 문헌뿐).
- **복구**: 학습 복구 실행기는 단독 채택하지 않는다(FAR 참고, MED-LOW: 단순 재시도 +12.1%p, 학습 추가분 +4.3%p). 복구의 "무엇을"은 Jev·Astra·M9가 정하고, 복구 동작(`resume_ckpt_k` 재접근 등)을 실행할 때도 R이 같은 제한으로 켜진다(M9 §4.1).
- **보조 역할 = 그림자 예측기**: 실행하지 않고 옆에서 돌려 진행도·phase 끝 확률·OOD 점수를 M7 소프트 채널 **후보**로 낸다(M7 §4.2, E-M7 조건 D-AE, 내부 절제만). M4 (b)에는 넣지 않는다.
- **제외**: Jev 보기 후보 생성은 Jev 역할을 바꾸므로 제외. 학습 추종 실행기는 아래 B와 같고, 명령을 맥락에 맞게 재해석할 위험(Steerable Policies 2602.13193, MED: "servos left towards an object, rather than moving left unconditionally")이 Jev 크기 권위를 넘을 수 있어 주 시스템에 넣지 않는다.
- **공정성**: R은 실행기 부품이라 모든 결정 층 조건에 똑같이 붙인다. B3c가 연속 행동을 내면 같은 방향·크기 격자로 양자화해 R을 붙인다(Claude 결정, EVAL §3.2-11). 주 표는 S, S+R은 병기 표·절제.
- **지연 [가정]**: 작은 MLP를 제어 주기마다 돌린다. 1 ms 미만 목표, 실측 필요.
- **실험**: E-R0~E-R4(`E-first-experiments.md` §4.16).

##### 비교 조건 B (정본 §33 설계, E-AE-2 표 전용 — 주 시스템에는 없음)
- **옛 줄(정본 §32·§33, 이제 B 조건에만 해당)**: 4.1.3의 "phase 실행"(확정된 Jev 답을 인자로 받아 짧게 움직이는 부분)은 **학습 실행기 B 하나가 모든 스킬의 모든 phase에서** 맡는다(user-log 33 보충 "스킬들의 모든 실행이 거기들어가 있어야한단뜻", user-log 34 "B로"). **결정 층(Astra 계획, Jev 선택)은 학습하지 않고 바깥에 둔다**(정본 §32).
- **바뀌지 않는 것(코드)**: FSM 전이는 계속 코드 술어 사건이 결정한다(실행기는 전이를 정하지 않는다). 스킬 계약(`entry`/`exit`/`invariants`/`effect`/`stop`/`budget`/`default_on_timeout`)과 진단 필드(최고 들어올림 높이, 최소 그리퍼 간격 등)는 **센서 측정으로 코드가 계산**하므로 그대로다. 결정 지점 id와 질문 틀(§4.1.2)도 그대로다.
- **B 조건에서 스크립트 스킬의 역할**:
  - **GT 생성기**(스킬 × phase × 보기마다): ① standard 장면에서 스크립트 스킬 실행, 성공 에피소드만(phase 경계·id·`option_key`·target 기록) ② 섭동 주입(물체 자세·시작 자세·실행 중 밀기) ③ DAgger — 익스퍼트 롤아웃 상태에서 스크립트 스킬이 라벨(α 0.5 시작, VIRAL 값 [가정]) ④ 복구 구간은 섞어서만. random 장면은 시험 전용이므로 학습 데이터는 standard에서만 만든다.
  - **백업 실행기(폴백)**: 샘플 분산이 보정 문턱 초과 또는 M4 (b) DEVIATE 2회 연속 [가정]이면 그 phase를 스크립트 스킬로 넘긴다(청크 경계, M5 관성화). 폴백은 FAIL이 아니며 Astra 호출 규칙은 그대로, 폴백 비율은 모든 표에 기록.
...
- **출력**: 행동 청크(0.5 s 분량을 100 Hz로 보간, 약 10 Hz로 생성 [가정]) + 보조 머리 셋 — (i) phase 진행도 → M7 `C_stag` 보조 소프트 채널, (ii) phase 끝 술어 예측 → 보조 채널만, (iii) flow 샘플 분산 → OOD 신호(conformal 보정). 확정된 행동 청크가 M4 (b)의 `ref(t)`다(M4 §4.1).
- **근거(D19, 등급 그대로)**: 물체 crop과 스킬 문장을 조건으로 받는 DiT 행동 익스퍼트(HiVLA 2604.14125, MED)는 스킬 조건을 빼면 Hard 과제에서 8.8% 떨어졌다. 스크립트 스킬 조합으로 만든 성공 궤적만으로 학습한 모델이 공식 π0와 맞먹었다(InternData-A1 2511.16651, MED: "18 skills, 70 tasks", "matches the official π0 across 49 simulation tasks"). 반대로 깨끗한 시연만으로 학습하면 무작위 장면에서 크게 떨어진다(RoboTwin 2.0 2506.18088, ICML 2026, HIGH: π0 46.4 → 16.3, "data without domain randomization does not help"). 교사 궤적만 모방하면 스스로 실수를 못 고친다(VIRAL 2511.15200, MED: "fails to correct its own mistakes", 학생 롤아웃 섞기로 개선).
- **크기·지연 [가정]**: 동결 소형 시각 인코더(Knowledge Insulating 2505.23705 방향, MED-HIGH) + 약 50–100M flow DiT, 청크당 ≤ 30 ms 목표(참고: π0.5 익스퍼트 300M, SmolVLA 전체 0.45B).
- **커버리지 지표 [제안]**: (skill, phase, option_key) 칸 중 성공 에피소드 ≥ N이고 칸별 폐루프 성공률이 스크립트 대비 −2%p 이내인 비율.
- **한계**: "증류 정책이 원래 스크립트 스킬을 넘었다"는 강한 근거는 찾지 못했다 → B의 이득(세밀 보정·연속성·지연 흡수)은 가설이다. 실험은 E-AE-0~5(`E-first-experiments.md` §4.15). 정본 §35로 B는 주 시스템에서 빠지고 E-AE-2의 비교 표로만 남는다. 도메인 무작위화를 넣었으므로(D35) B 학습 데이터도 R·B3c와 같은 무작위화 데이터를 쓴다.

#### 4.1.5 AI Worker 양팔 스킬 (정본 §36~§38, `D21-aiworker-zed.md`) [제안]
- **로봇**: ROBOTIS AI Worker(user-log 37). 실물 = **FFW-SG2**(스워브 이동 베이스, 총 25자유도)를 **베이스 고정**으로 쓴다(정본 §38, user-log 38 "Sg2야"). 쓰는 것: 팔 7자유도 × 2, 그리퍼 RH-P12-RN(1자유도 평행, 0–107.6 mm) × 2, 머리 2자유도(pitch −50°~30°, yaw −20°~20°), 리프트 0–500 mm. 이동 6자유도는 쓰지 않는다. 제어: ROS 2 Jazzy, 팔마다 100 Hz JointTrajectoryController(위치 명령).
- **유지**: 기존 pick/place FSM과 `arms: left|right|both` 결정 지점(§7-4, §4.1.1 `harvest.arms`). 그리퍼는 1자유도 평행형이라 E §1.4 "7자유도 팔 + 평행 그리퍼" 가정과 한 팔 기준으로 그대로 맞는다.
- **추가 스킬**: `look_at(target)`(머리 2축 — 머리가 움직이므로 M8 격자 칸에 머리 각을 표시, M8 §4.3), `set_lift`(0–500 mm, 작업 높이 맞춤), `handover`(좌→우), `bimanual_hold`. 계약 형식·FSM·결정 지점 규칙은 §4.1.1~4.1.3 그대로다.
- **범위 밖**: 이동 베이스(SG2의 스워브).
- **로봇별과 공유**: 결정 층·술어 등록부·Jev/Astra 계약은 두 로봇(RoboDojo ARX X5, AI Worker)이 같은 코드이고, 스킬·M5 한계·R은 로봇별이다(EVAL §2.1 3층 정합). 위험: 7축과 6축, 머리 이동 유무, 베이스 간격이 달라 스킬 이식 비용이 두 벌이다(D21 §3.7).
...
10. 해소(정본 §28, D17, M3 §4.1과 같은 규약): 결정 지점 질문도 `question_id@vN` 판본(J1), 상태 직렬화 정규화(J2), 합의 표는 시간차 상태에서만(J3), 게이트·감시는 판본별(J4)(§4.1.2). 새 [결정 필요]는 없다.
11. 해소(정본 §32·§33, user-log 34 사용자 결정 B): phase 실행 = 학습 실행기 B, 스크립트 스킬 = GT 생성기 + 폴백, FSM 전이·계약·진단 필드는 코드(§4.1.4). 새 [결정 필요]는 SUMMARY §5.1 D35(도메인 무작위화 허용)·D36(주 표 실행기 S 대 B).
12. 해소(정본 §34·§35, user-log 35·36): D35 = 무작위화 넣음, D36 = 주 표 실행기 S. phase 실행 = S, action expert 주 역할 = 구간 제한 잔차 R(코드 투영으로 Jev 방향·크기 구간 권위 유지), 보조 = 그림자 예측기(M7 후보), 학습 복구는 R에 흡수, 후보 생성 제외, B는 E-AE-2 비교 표로만(§4.1.4). 새 사용자 [결정 필요]는 없다.
13. 해소(정본 §36~§39, user-log 37~39): 4번 양팔 — AI Worker 기준으로 `arms` 결정 지점 유지 + 스킬 `look_at`·`set_lift`·`handover`·`bimanual_hold` 추가, 이동 베이스는 범위 밖, 실물 = FFW-SG2 베이스 고정(§4.1.5). R의 힘 입력 = 관절 `effort`(전류) + 그리퍼 전류, 손목 F/T 추가 안 함, 절제 R-noforce(§4.1.4). 남은 하드웨어 [결정 필요]는 없다.
...
- (정본 §37, D21) SH5 손의 판매 상태, 사양 페이지 D405와 URDF 파일 이름 `wrist_d401.urdf.xacro`의 차이 이유 [미확인].
