...
#### 4.1.4 phase 실행 = 학습 실행기 B (정본 §32·§33, `D19-action-expert.md`) (사용자 결정 B, 세부는 [제안]·[가정])
- **바뀐 것**: 4.1.3의 "phase 실행"(확정된 Jev 답을 인자로 받아 짧게 움직이는 부분)은 **학습 실행기 B 하나가 모든 스킬의 모든 phase에서** 맡는다(user-log 33 보충 "스킬들의 모든 실행이 거기들어가 있어야한단뜻", user-log 34 "B로"). **결정 층(Astra 계획, Jev 선택)은 학습하지 않고 바깥에 둔다**(정본 §32).
...
- **한계**: "증류 정책이 원래 스크립트 스킬을 넘었다"는 강한 근거는 찾지 못했다 → B의 이득(세밀 보정·연속성·지연 흡수)은 가설이다. 실험은 E-AE-0~5(`E-first-experiments.md` §4.15).
