...
전제: M1은 [결정 필요]다. 이 문서는 M3·M7처럼 **M1 후보 1(코드가 인식·기하로 범주형 술어를 만든다)을 가정**한다.
...
|---|---|---|---|---|---|---|
| **Agent Skills 명세** (SKILL.md, agentskills.io) ◎ | LLM 에이전트 표준 | 성능 수치가 아니라 **사실상 표준**: Anthropic이 만들어 2025-12-18 공개 표준으로 냄(2차 출처 firecrawl 블로그, 날짜는 원문 미확인). 명세: 필수 `name`(≤64자), `description`(≤1024자, "무엇을·언제"), 선택 `compatibility`, `metadata`, `allowed-tools`(실험). **점진 공개 3단**: 메타데이터 ~100토큰은 시작 때 전부, 본문 <5000토큰 권장은 활성화 때, `scripts/`·`references/`·`assets/`는 필요할 때. 본문 500줄 이하 | HIGH(표준, 다수 제품 채택) — 논문 아님 | 2025-12 | 예 | 아니오(PhyAgentOS가 SKILL.md를 절차 기억으로 씀 ◎) |
| **MCP Tools 명세 2025-06-18** ◎ | LLM 도구 표준 | `inputSchema`(JSON Schema), 선택 `outputSchema`(있으면 서버는 반드시 준수, 클라이언트는 검증해야 함), `structuredContent`, 오류 두 층(프로토콜 오류 대 `isError: true` 실행 오류), `annotations`: `readOnlyHint`, `destructiveHint`, `idempotentHint`, `openWorldHint`(schema.ts). "annotations는 신뢰된 서버가 아니면 **untrusted**로 취급" | HIGH(표준) | 2025-06 | 예 | ROSClaw 등 로봇 MCP 서버 있음(v3/06) |
...
| **BATON** 전이 인지 계약 ◎ | 로봇 | "VLA primitive carries an **exit** condition but no **entry** condition" 을 지적. 세 전이: invocation(손목 카메라로 준비 확인 뒤에만 VLA 호출), handoff(앞 단계 잔여물이 흐트린 진입 상태 복구), **lookahead**(뒤 단계가 요구하는 조건이 지금 단계의 실행 방식을 정함). RoboMemArena 과제 성공 +11.6%, 누적 +14.9%(SoTA 대비, 초록) | LOW-MED(USC, 심사 전, 2026-08-17, 반응 미측정) | 2026-08 | 예(파라미터 갱신 없음) | 예 |
| **GPSFSM** (2607.15674) ○ | 로봇 | LLM이 부분 명세 FSM(상태, **사건 촉발 전이**, Sequential/Recovery/Parallel-Any/All)을 생성, 엔진이 파싱·검증·실행, ROS 2 Capabilities2에 **실행 시 파라미터 주입**·비동기 사건. 항법에서 BTGenBot보다 계획 생성 성공률 일관되게 높음(초록, 수치 미확인) | MED-HIGH(**IROS 2026 채택**, arXiv 코멘트) | 2026-07 | 예 | 예 |
...
| BATON ◎ | 전이(invocation/handoff/lookahead)마다 검증 에이전트 | 중간(경계) | 본문 |
...
| MCP Tools | `inputSchema`/`outputSchema`, `isError`(실행 오류) 대 프로토콜 오류, annotations 4종은 "힌트이며 신뢰 전 untrusted" | 스킬 계약을 JSON Schema로: 입력 = **범주형 파라미터 enum**(보기 ID), 출력 = 결과 술어 + 오류 코드(`isError` 대응). annotations를 로봇 의미로 옮김: `destructiveHint` → `irreversible`(놓기·삽입 시작: M4 확정 전 실행 금지, M9 잠금 대상), `idempotentHint` → `retry_safe`(M9 L1 재시도 허용), `readOnlyHint` → 인식·관찰 스킬. **"힌트는 untrusted"를 그대로**: 되돌릴 수 없음 여부는 스킬 작성자 선언이 아니라 코드 안전 규칙이 최종 |
...
| PhyAgentOS | 세션 계약: 사전조건·실행 한계·수용 기준. SKILLRUNTIME.md: 필요한 관측·행동 형식·파라미터. 교훈은 "provenance and scope"와 함께, 사전조건이 맞을 때만 전이 | 스킬 계약 필드 `pre`(진입), `accept`(수용 = 출구), `limits`. 교훈 주입 조건은 M10의 정확 일치 키와 같은 원리 |
| BATON | 출구만 있고 진입이 없는 문제, handoff·lookahead 전이 | **스킬마다 `entry`와 `exit`를 둘 다**, 그리고 **`lookahead_req`**(다음 스킬의 `entry`가 요구하는 것 중 지금 스킬 실행 방식에 걸리는 것, 예: 놓을 자리가 좁으면 옆잡기). 계획 시점에 코드가 `exit_k ⇒ entry_{k+1}` 검사, 성립하지 않으면 Astra에 되돌림. Jev의 접근 방향 질문 보기에 "뒤 단계 호환" 술어를 붙인다 |
| Zetta | critic 제안 (증거, 모드), 승인자만 개입 허용, 재진입 = 증거 해소 AND 접촉 안정 | Jev 질문 `Q_critic_accept`: critic 제안을 {수락, 거절-계속, 올림}. 재진입 조건을 스킬 계약 `reentry` 필드로(코드 술어). 원문 승인자는 LLM(Claude/GPT, v3/16 #23)이고 **Jev로 바꾸는 것은 접목** |
| GPSFSM | 사건 촉발 전이, Recovery 구조, 실행 시 파라미터 주입 | 스킬 **내부 단계 = 작은 FSM**(approach→align→grasp→lift/transport→place→retreat), 전이는 코드 술어 사건으로, 파라미터는 전이 순간 Jev 답으로 주입. BT 대신 FSM을 기본으로 둘지는 E-M6-2에서 |
...
| SkillWrapper | 블랙박스 스킬의 사전조건·효과를 기반 모델로 발명·학습 | "기존 스킬 = 라이브러리(a)"일 때 `entry`/`exit` 술어가 없는 스킬에 **오프라인으로 술어를 붙이는 방법** 후보(대안). 1순위는 사람이 쓰고 Astra가 제안 |
...
// contract.json (MCP inputSchema/outputSchema 형식을 빌림)
{
  ...
  "entry":  ["reachable(target)", "gripper_open", "not holding(any)"],   // 진입 술어 (BATON)
  "exit":   ["holding(target)", "lifted(target, >=3cm)"],                // 출구 = 수용 기준 (PhyAgentOS)
  "invariants_by_phase": { "transport": ["holding(target)"] },           // M7 inv_k로 넘어감
  "phases": ["approach", "align", "grasp", "lift"],
  "decision_points": ["dp.approach_dir", "dp.align_commit", "dp.grasp_commit", "dp.grasp_result"],
  "stop":   { "approach": "dist_to_pregrasp < tol", "align": "aligned_xy & aligned_yaw" },
  "budget": { "approach": "4s", "align": "3s", "grasp": "2s", "lift": "2s" },
  "reentry": ["failure_evidence_cleared", "contact_stable"],             // Zetta
  "lookahead_req": { "if next=place-narrow": "approach=side_*" },        // BATON
...
- Astra 권한: 계약에 있는 스킬·보기만 고른다(Harness VLA "cannot invent"). 새 스킬 제안은 오프라인 경로(M10 검증 게이트)로만.

#### 4.1.2 Jev 질문을 꽂는 자리: 단계별 템플릿 [제안]
원칙: (1) 코드 술어로 답이 나오는 것은 묻지 않는다. (2) 질문은 **단계 전이 사건** 때와 M4의 겹침 확인 때만. (3) 모든 보기에 예상 결과 술어(M3), 모든 질문에 `NONE_ESCALATE`. (4) 같은 문구 고정(Jev #8). (5) 확률은 게이트(보정 E1 전).
...
| approach | `dp.approach_dir` (스킬 진입 시, 장애물 술어 변화 시) | "Choose the approach for {target}." | `top`, `side_front`, `side_left`, `side_right` 각각 `after: reachable=?, collision_risk=?, next_entry_ok=?`(lookahead) | 스킬 인자 |
...
| grasp | `dp.grasp_result` (**코드 술어가 애매할 때만**: 그리퍼 폭이 "빈 집기"와 "얇은 물체" 경계) | "What is the grasp outcome?" | `secure`, `slipping`, `empty`, `uncertain` | M7 입력(S5), 확정은 코드 우선 |
| lift/transport | `dp.critic_accept` (critic 제안 사건) | "The monitor proposes {mode} because {evidence}. Accept?" | `accept`, `reject_continue`, `NONE_ESCALATE` | Zetta 승인자 자리(접목) |
...
#### 4.1.3 스킬 실행 쪽 (코드)
단계 FSM(GPSFSM식): 상태 = phase, 전이 = 코드 술어 사건(stop 참, budget 초과, inv 위반, critic 제안). 전이 순간에 해당 결정 지점의 **확정된** Jev 답(M4 출력)을 파라미터로 주입. 확정 답이 없으면 계약의 `default_on_timeout`(보통 "현재 단계 유지 + 안전 대기").

### 4.2 대안 A: 경계만(BATON·Harness VLA식)
...
|---|---|---|---|---|---|
| (a) 보유 라이브러리 | 벤치마크·로봇에 이미 있는 스킬(예: RoboDojo·LIBERO 제공 프리미티브, 모션 플래너 pick/place) | 스킬이 단계 훅을 안 주면 **경계만**(대안 A). 훅을 주려면 스킬을 단계로 나누는 래퍼 필요 | 사람(또는 SkillWrapper식 오프라인 발명) | 스킬 내부 실패를 못 봄. "가장 약한 노드"(2603.01113)가 그대로 | Harness VLA, BATON |
| (b) 생성 스킬 | Astra/코드 에이전트가 과제마다 스킬 코드를 생성(CaP-X식) | 코드 어디든 `jev_choice(dp_id)` 삽입 가능 → **가장 잘게** | Astra가 생성, 계약 검증기(코드)가 형식·술어 검사 | **SkillsBench(2602.12670 v4): 스스로 만든 스킬이 스킬 없음보다 −8.1~−11.5pp**(경험 없이 사전 생성 조건). 생성 코드의 오류가 로봇 동작으로 | CaP-X, SkillsBench(v3/03) |
| (c) 새 라이브러리 | 우리가 단계 FSM·계약을 갖춘 스킬을 새로 작성 | 설계대로 전부 | 사람 | "기존 스킬"이라는 사용자 표현과 멀어짐, 작성 비용, 비교 공정성(우리만 좋은 스킬) | — |
| **(a') 1순위 제안** | (a)를 단계 래퍼로 감싸 계약을 붙인 것. 스킬 몸체는 그대로, 단계 경계에 훅 | 단계 경계 + 인자 | 사람 작성, Astra는 `lookahead_req`만 계획 시점에 채움 | 래퍼가 안 되는 스킬(학습 정책 한 덩어리)은 대안 A로 | — |

**사용자 표현("스킬을 생성해서 잡는")은 (b)에 가깝다.** 그래서 [결정 필요]: (a')를 본 조건으로, (b)를 "기존 방법 재현 + Jev 결정 지점 삽입" 조건으로 둘 다 실험할지, (b)를 본 조건으로 할지. 두 안 모두 계약 형식(4.1.1)은 같다 — (b)면 Astra가 계약까지 생성하고 코드가 검증한다.

---

## 5. 비교 실험 (판정 기준은 실행 전 고정)

공통: 같은 M1 술어·같은 스킬 몸체·같은 Astra 계획(고정 계약 파일 재사용)·Jev 버전 고정. 과제 = 단일 pick-and-place + 2단계 연쇄(pick→narrow place) + 섭동 4종(물체 이동, 방해물 삽입, 미끄러짐 주입, 놓을 자리 좁힘). 시드 ≥ 3(시드 = 장면 배치 + 섭동 시각), **평균 ± 표준편차**. 시뮬(RoboDojo-Sim 또는 LIBERO 계열, plan [결정 필요] 3 따름).
...
| G3 | G2 + `dp.critic_accept`(Zetta 자리) |
...
### E-M6-2 인터페이스 절제
G2에서 하나씩 뺀다: −예상 결과 술어 / −`entry`(BATON) / −`lookahead_req` / −`stop` 술어(budget만) / −`NONE_ESCALATE` / FSM 대신 BT(대안 B). 판정: 빼서 성공률이 **3%p 이상** 떨어지는 필드만 필수로 남긴다. `lookahead_req`는 "narrow place" 과제에서만 판정.

### E-M6-3 "기존 스킬" 정의
(a') 대 (b)(Astra 생성 + `jev_choice` 삽입) 대 (b) 원형(생성 코드만, Jev 없음 = CaP-X식 기준). 지표에 **생성 코드 형식 오류율, 계약 검증 거부율** 추가. 판정: (b)가 (a')보다 낮으면 SkillsBench 경고가 우리에게도 해당된다고 보고.
...
- **SkillsBench(v3/03, v4)**: 사전 생성 스킬이 스킬 없음보다 나쁨(−8.1~−11.5pp). (b)안의 위험.
...
- BATON·GPSFSM 수치는 초록만 봤다. BATON은 심사 전.
- Zetta 원문 불일치(온라인 LLM 승인자 대 인프라 절 "온라인 에이전트 없음", v3/16 #23).

## 7. 열린 질문, [결정 필요]
1. [결정 필요] "기존 스킬"의 정의: (a') 본 조건 + (b) 비교, 또는 (b) 본 조건(4.4).
...
5. `lookahead_req`를 Astra가 계획 때 채울지, 스킬 작성자가 표로 둘지.
...
- GPSFSM의 BTGenBot 대비 수치, BATON 표 수치, SkillWrapper 학회.
