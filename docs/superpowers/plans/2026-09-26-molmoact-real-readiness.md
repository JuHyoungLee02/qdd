...
- **변경 2(2026-09-26 06:20 UTC, G-trace 눈 검사 뒤)**: 놓기 프레임 시트(`release_sheet_v2.jpg`)에서 RB1 ep366 k11·RB2 ep524 k49처럼 '놓기'가 실제로는 **편 시작부터 닫혀 있던 그리퍼가 쥐기 전에 여는 것**이었다(궤적 끝이 놓을 곳이 아니라 집을 물체). 놓기 = 닫힘 → 열림 중 **그 닫힘이 편 안에서 시작된 것**(열림 → 닫힘이 앞에 있음)만(`grip_segment_ends(require_grasp=True)`). 거르개 결정·눈 검사 판정은 구간 끝과 무관해 그대로; 라벨·통계·시트만 다시 만든다.
